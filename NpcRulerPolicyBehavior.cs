...
public sealed class NpcRulerPolicyBehavior : CampaignBehaviorBase
{
	...
	private static NpcPolicyPrompt ComposePolicyPrompt(NpcRulerPolicyBatchContext context, string editablePrompt)
	{
		...
		contract.AppendLine("subjectKind 只允许 rulerFiefs、vassalFiefs、allKingdomFiefs、rulerClan、vassalClans、allMemberClans；minClanTier/maxClanTier 可按家族等级 0-6 限制主体。定居点、税收、建造和志愿兵指标只能配领地主体；clanInfluenceDailyDelta 必须配氏族主体，并与领地指标拆成不同 effect；kingdomStabilityDailyDelta 不能指定局部主体，必须使用 allKingdomFiefs，可与同主体领地效果写在同一 effect。必须先判断统治者与封臣之间的直接因果关系：既有权力、税源、兵源控制、行政权限或政治影响力的相对再分配，通常是一方增强而另一方削弱或至少不受益；新增资源、共同投资、制度增效、全国增长或协同动员可以让双方同时增强且增幅不同；混合政策可以在部分指标共同增强、同时在影响力或控制权上此消彼长。不得机械零和、不得自动复制相反数、不得为了对称凭空补效果；某一方没有直接变化时应省略，最终关系、方向、指标与数值全部按政策具体措施判断。");
