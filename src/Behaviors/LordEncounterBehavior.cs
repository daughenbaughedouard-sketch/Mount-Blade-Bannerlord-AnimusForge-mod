...
public class LordEncounterBehavior : CampaignBehaviorBase
{
	...
	private void AddGameMenus(CampaignGameStarter starter)
	{
		...
		starter.AddGameMenuOption("AnimusForge_lord_encounter", "native_dialogue_lord", "原版对话", delegate(MenuCallbackArgs args)
		{
			if (HasPendingForceNativeDefeatCaptivityMenu())
			{
				return false;
		...
		starter.AddGameMenuOption("AnimusForge_lord_encounter", "meet_lord", "进入军团对话", delegate(MenuCallbackArgs args)
		{
			if (HasPendingForceNativeDefeatCaptivityMenu())
			{
				return false;
		...
		}, delegate(MenuCallbackArgs args)
		{
			...
			if (hero == null)
			{
				Logger.Log("LordEncounter", "Army dialogue option clicked but target hero is null after refresh.");
				InformationManager.DisplayMessage(new InformationMessage("当前未识别到遭遇领主，请先离开并重新接触。", Colors.Yellow));
				return;
			}
			...
			List<Hero> availableLords = GetTargetHeroes();

			// 如果只有一个领主，直接开始会面
			if (availableLords.Count <= 1)
			{
				IsOpeningConversation = true;
				try
				{
					StartMeetingWithSelectedLords(hero, availableLords, args);
				}
				finally
				{
			...
			{
				// 多个领主时，显示选择UI
				ShowLordSelectionScreen(hero, availableLords, args);
			}
		});
	...
	private static void ShowLordSelectionScreen(Hero primaryHero, List<Hero> availableLords, MenuCallbackArgs args)
	{
		try
		{
			Logger.Log("LordEncounter", $"[LordSelection] Showing lord selection screen with {availableLords.Count} lords");

			bool shown = LordSelectionScreen.Show(
				availableLords,
				(selectedLords) =>
				{
					// 用户确认选择
					Logger.Log("LordEncounter", $"[LordSelection] User selected {selectedLords.Count} lords");
					IsOpeningConversation = true;
					try
					{
						StartMeetingWithSelectedLords(primaryHero, selectedLords, args);
					}
					finally
					{
						IsOpeningConversation = false;
				...
				{
					// 用户取消
					Logger.Log("LordEncounter", "[LordSelection] User cancelled lord selection");
					InformationManager.DisplayMessage(new InformationMessage("已取消会面", Colors.Yellow));
				}
			);
			if (!shown)
			{
				Logger.Log("LordEncounter", "[LordSelection] Popup failed to open; falling back to all lords.");
				IsOpeningConversation = true;
				try
				{
					StartMeetingWithSelectedLords(primaryHero, availableLords, args);
				}
				finally
				{
					IsOpeningConversation = false;
		...
		{
			Logger.Log("LordEncounter", $"[LordSelection] Failed to show selection screen: {ex.Message}");
			// 如果UI失败，回退到使用所有领主
			IsOpeningConversation = true;
			try
			{
	...
	private static void StartMeetingWithSelectedLords(Hero primaryHero, List<Hero> selectedLords, MenuCallbackArgs args)
	{
		try
		{
			if (primaryHero == null)
			{
				Logger.Log("LordEncounter", "StartMeetingWithSelectedLords aborted: primary hero is null");
				return;
			}

			if (selectedLords == null)
			{
				selectedLords = new List<Hero>();
			}
			selectedLords = selectedLords.Where((Hero h) => h != null && h != Hero.MainHero && h.IsLord).Distinct().ToList();
			if (selectedLords.Count == 0)
			{
				Logger.Log("LordEncounter", "StartMeetingWithSelectedLords: No lords selected, using primary hero only");
				selectedLords = new List<Hero> { primaryHero };
			}
			Hero meetingPrimary = selectedLords.Contains(primaryHero) ? primaryHero : selectedLords[0];

			// 设置选中的领主
			SetTargets(selectedLords);
			SetPrimaryTarget(meetingPrimary);

			selectedLords = GetTargetHeroes().ToList();

			Logger.Log("LordEncounter", $"[LordSelection] Starting meeting with {selectedLords.Count} selected lords. Primary: {meetingPrimary.Name}");

			try
			{
				InformationManager.DisplayMessage(new InformationMessage($"[AF] 会面领主数: {selectedLords.Count}（主目标: {meetingPrimary.Name}）", Colors.Yellow));
			}
			catch
			{
			}
			...
			{
				IFaction faction = PartyBase.MainParty?.MapFaction;
				IFaction faction2 = meetingPrimary?.MapFaction;
				_lastMeetingWasSameMapFactionConflict = faction != null && faction2 != null && faction == faction2;
				if (faction?.Name != null)
				{
					_lastMeetingPlayerFactionName = faction.Name;
			...
			MeetingBattleRuntime.BeginMeeting(selectedLords);
			Campaign.Current.CurrentConversationContext = ConversationContext.PartyEncounter;
			SaveMainPartyPosition();

			if (args == null)
			{
				Logger.Log("LordEncounter", "StartMeetingWithSelectedLords aborted: menu args are null");
				return;
			}

			DisableMeetingSpawnOverride();
			Logger.Log("LordEncounter", "Meeting requested: redirecting to native encounter attack consequence");
			EnsureEncounterBattlePrepared(meetingPrimary);
			LordEncounterRedirectGuard.SuppressForSeconds(8f);
		...
		{
			Logger.Log("LordEncounter", $"StartMeetingWithSelectedLords failed: {ex2}");
			MeetingBattleRuntime.EndMeeting();
			DisableMeetingSpawnOverride();
		}
	}
