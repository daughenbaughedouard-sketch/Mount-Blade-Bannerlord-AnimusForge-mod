	...
	public static async Task<string> CallApiWithMessages(List<object> messages, int maxTokens, bool recordTokenStats = true)
	{
		...
		try
		{
			...
			JObject payload = new JObject
			{
				...
				["max_tokens"] = JToken.op_Implicit(5000)
			};
			...
			foreach (object msg in messages)
			{
				...
				val["role"] = (JToken)dict.role;
				val["content"] = (JToken)dict.content;
				messagesArray.Add((JToken)(object)val);
			}
			payload["messages"] = (JToken)(object)messagesArray;
			string jsonBody = ((JToken)payload).ToString((Formatting)0, Array.Empty<JsonConverter>());
			...
			try
			{
				...
				if (response.IsSuccessStatusCode)
				{
					try
					{
						...
						string content = ((string)((JToken)responseJson).SelectToken("choices[0].message.content")) ?? ((string)((JToken)responseJson).SelectToken("content")) ?? ((string)((JToken)responseJson).SelectToken("text"));
	...
	public static async Task CallApiWithMessagesStream(List<object> messages, int maxTokens, Action<string> onChunk, Action<string> onComplete, Action<string> onError, CancellationToken cancellationToken = default(CancellationToken))
	{
		...
		try
		{
			...
			JObject payload = new JObject
			{
				["model"] = JToken.op_Implicit(settings.ModelName),
				["max_tokens"] = JToken.op_Implicit(5000),
				["stream"] = JToken.op_Implicit(true)
			};
			JArray messagesArray = new JArray();
			foreach (object msg in messages)
			{
			...
			for (int attempt = 1; attempt <= 2; attempt++)
			{
				...
				try
				{
					...
					try
					{
						...
						{
							string text2;
							string text = (text2 = await reader.ReadLineAsync());
							string line = text2;
							...
							try
							{
								...
								string delta = (string)((JToken)chunk).SelectToken("choices[0].delta.content");
								if (delta == null)
								{
									delta = (string)((JToken)chunk).SelectToken("delta.content");
								}
								if (delta == null)
								{
									delta = (string)((JToken)chunk).SelectToken("content");
								}
								if (delta == null)
								{
									delta = (string)((JToken)chunk).SelectToken("text");
								}
