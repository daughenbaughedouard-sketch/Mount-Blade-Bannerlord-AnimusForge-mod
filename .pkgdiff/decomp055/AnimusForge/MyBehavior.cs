...
public class MyBehavior : CampaignBehaviorBase
{
	...
	private void OnDailyTick()
	{
		try
		{
			...
			DuelSettings settings = DuelSettings.GetSettings();
			if (settings != null && settings.AutoGenerateWeeklyReports && currentGameDayIndexSafe > 0 && currentGameDayIndexSafe % 7 == 0 && num > 0 && _lastAutoGeneratedWeeklyReportWeek < num)
			{
	...
	private void StartAutoWeeklyReportsForWeek(int weekIndex, int currentGameDayIndexSafe)
	{
		if (weekIndex > 0 && _lastAutoGeneratedWeeklyReportWeek < weekIndex && !_weeklyReportGenerationInProgress)
		{
			...
			GenerateAutoWeeklyReportsAsync(groups, weekIndex, startDay, endDay);
		}
	}
	...
	private static int GetKingdomStabilityWeeklyBalancingDelta(int stabilityValue)
	{
		return GetKingdomStabilityTier(stabilityValue) switch
		{
			KingdomStabilityTier.ExtremelyHigh => -5, 
			KingdomStabilityTier.High => -3, 
			KingdomStabilityTier.VeryPoor => 3, 
			KingdomStabilityTier.ExtremelyPoor => 5, 
			_ => 0, 
		};
	}
	...
	private void QueueAutomaticKingdomRebellion(KingdomRebellionResolutionResult result)
	{
		if (result?.Kingdom != null && result.SelectedClan != null)
		{
			...
			PendingAutomaticKingdomRebellionContext pendingAutomaticKingdomRebellionContext = new PendingAutomaticKingdomRebellionContext();
			pendingAutomaticKingdomRebellionContext.KingdomId = GetKingdomId(result.Kingdom);
			pendingAutomaticKingdomRebellionContext.ClanId = GetClanId(result.SelectedClan);
			pendingAutomaticKingdomRebellionContext.WeekIndex = result.WeekIndex;
			pendingAutomaticKingdomRebellionContext.StabilityValue = result.StabilityValue;
			pendingAutomaticKingdomRebellionContext.StabilityTierText = result.StabilityTierText;
			pendingAutomaticKingdomRebellionContext.RelationToKing = kingdomRebellionCandidateInfo?.RelationToKing ?? 0;
			pendingAutomaticKingdomRebellionContext.TownCount = kingdomRebellionCandidateInfo?.TownCount ?? 0;
			pendingAutomaticKingdomRebellionContext.CastleCount = kingdomRebellionCandidateInfo?.CastleCount ?? 0;
			pendingAutomaticKingdomRebellionContext.FollowerClanIds = (from x in (result.SelectedFollowerClans ?? new List<Clan>()).Where((Clan x) => x != null && x != result.SelectedClan).Select(GetClanId)
				where !string.IsNullOrWhiteSpace(x)
				select x).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
			PendingAutomaticKingdomRebellionContext item = pendingAutomaticKingdomRebellionContext;
	...
	private void TryStartNextAutomaticKingdomRebellionAsync()
	{
		...
		Kingdom val = FindKingdomById(pendingAutomaticKingdomRebellionContext.KingdomId);
		Clan clan = FindClanById(pendingAutomaticKingdomRebellionContext.ClanId);
		List<Clan> followerClans = (from x in (pendingAutomaticKingdomRebellionContext.FollowerClanIds ?? new List<string>()).Select(FindClanById)
			where x != null && x != clan
			select x).ToList();
		if (val == null || clan == null)
		{
			ShowAutomaticKingdomRebellionCompletionPopup(pendingAutomaticKingdomRebellionContext, val, clan, followerClans, success: false, "本周自动叛乱已命中，但目标王国或家族状态已变化，无法继续执行。");
			return;
		}
		BuildRebelKingdomNamingRequest(clan, val, pendingAutomaticKingdomRebellionContext.WeekIndex, followerClans, out var systemPrompt, out var userPrompt, out var fallbackResult);
		...
		Task.Run(delegate
		{
			RebelKingdomNamingResult namingResult = GenerateRebelKingdomNamingFromPrompts(systemPrompt, userPrompt, fallbackResult, "自动叛乱建国命名 - " + GetClanId(clan));
	...
	private void ProcessPendingAutomaticKingdomRebellionResult()
	{
		...
		if (pendingAutomaticKingdomRebellionContext != null)
		{
			...
			{
				success = TryExecuteKingdomRebellionWithNaming(clan, val, pendingAutomaticKingdomRebellionContext.WeekIndex, forceTrigger: false, pendingAutomaticKingdomRebellionContext.RelationToKing, pendingAutomaticKingdomRebellionContext.TownCount, pendingAutomaticKingdomRebellionContext.CastleCount, pendingAutomaticKingdomRebellionContext.NamingResult, followerClans, out message);
			}
			ShowAutomaticKingdomRebellionCompletionPopup(pendingAutomaticKingdomRebellionContext, val, clan, followerClans, success, message);
		}
	}
	...
	private void ShowAutomaticKingdomRebellionCompletionPopup(PendingAutomaticKingdomRebellionContext context, Kingdom kingdom, Clan clan, List<Clan> followerClans, bool success, string executionMessage)
	{
		...
		InformationManager.ShowInquiry(new InquiryData(success ? "自动叛乱执行完成" : "自动叛乱执行失败", stringBuilder.ToString().TrimEnd(), true, false, "继续", "", (Action)delegate
		{
	...
	{
		if (_missingOnnxGateActive && Campaign.Current != null && Campaign.Current.GameStarted && !InformationManager.IsAnyInquiryActive() && DateTime.UtcNow.Ticks >= _missingOnnxGateResumeAfterUtcTicks)
		{
			ShowMissingOnnxGatePopup();
		}
	}
	...
	private void ShowMissingOnnxGatePopup()
	{
		...
		InformationManager.HideInquiry();
		InformationManager.ShowInquiry(new InquiryData("缺少ONNX文件", "检测到你的mod缺乏ONNX文件，请前往群文件下载RAG专用模型，并将里面的onnx拖入AnimusForge的mod文件中", true, false, "退出存档", "", (Action)ExitCurrentGameBecauseOnnxMissing, (Action)null, "", 0f, (Action)null, (Func<ValueTuple<bool, string>>)null, (Func<ValueTuple<bool, string>>)null), true, false);
	}
	...
	private static bool HasCompleteRequiredOnnxFiles()
	{
		try
		{
			...
			string value = FindFirstExistingFile(Path.Combine(text, "tokenizer.json"), Path.Combine(text2, "tokenizer.json"));
			if (string.IsNullOrEmpty(value))
			{
				return false;
			}
			string value2 = FindFirstExistingFile(Path.Combine(text, "config.json"), Path.Combine(text2, "config.json"));
			if (string.IsNullOrEmpty(value2))
			{
				return false;
			}
			string value3 = FindFirstExistingFile(Path.Combine(text2, "model_quantized.onnx"), Path.Combine(text, "model_quantized.onnx"));
			if (!string.IsNullOrEmpty(value3))
			{
				return true;
			}
			string text3 = FindFirstExistingFile(Path.Combine(text2, "model.onnx"), Path.Combine(text, "model.onnx"));
			if (string.IsNullOrEmpty(text3))
			{
				return false;
			}
			return File.Exists(text3 + "_data");
		}
		catch
		{
			return false;
	...
	private static string PrependExtraRuleDisclaimer(string text)
	{
		...
		if (text.IndexOf("【说明】你不必提到附加规则内的内容，除非有人问起。", StringComparison.OrdinalIgnoreCase) >= 0)
		{
			return text;
		}
		return "【说明】你不必提到附加规则内的内容，除非有人问起。" + Environment.NewLine + text.TrimStart();
	}
	...
	{
		return kingdom != null && !kingdom.IsEliminated && !string.IsNullOrWhiteSpace(((MBObjectBase)kingdom).StringId);
	}
	...
	private static bool IsWeeklyReportGroupEligible(WeeklyEventMaterialPreviewGroup group)
	{
		...
		string a = (group.GroupKind ?? "").Trim();
		if (string.Equals(a, "world", StringComparison.OrdinalIgnoreCase))
		{
			return true;
		}
		if (!string.Equals(a, "kingdom", StringComparison.OrdinalIgnoreCase))
		{
			return false;
		}
				...
				foreach (EventMaterialReference material in item.Materials)
				{
					if (material != null)
					{
						eventRecordEntry.Materials.Add(new EventMaterialReference
						{
							...
							RelatedHeroIds = new List<string>(from x in material.RelatedHeroIds ?? new List<string>()
								where !string.IsNullOrWhiteSpace(x)
								select x.Trim()),
							RelatedClanIds = new List<string>(from x in material.RelatedClanIds ?? new List<string>()
								where !string.IsNullOrWhiteSpace(x)
								select x.Trim()),
							RelatedKingdomIds = new List<string>(from x in material.RelatedKingdomIds ?? new List<string>()
								where !string.IsNullOrWhiteSpace(x)
								select x.Trim()),
							SourceStableKeys = new List<string>(from x in material.SourceStableKeys ?? new List<string>()
								where !string.IsNullOrWhiteSpace(x)
								select x.Trim()),
							SourceActionKinds = new List<string>(from x in material.SourceActionKinds ?? new List<string>()
								where !string.IsNullOrWhiteSpace(x)
								select x.Trim()),
	...
	private void OpenDevEventViewerMenu(int page)
	{
		...
		{
			list2.Add(new InquiryElement((object)item, BuildDevEventRecordMenuLabel(item), (ImageIdentifier)null));
		}
	...
	private static string BuildDevEventRecordMenuLabel(EventRecordEntry entry)
	{
		...
		bool flag = string.Equals((entry.EventKind ?? "").Trim(), "world", StringComparison.OrdinalIgnoreCase);
		string text = ResolveKingdomDisplay(entry.ScopeKingdomId);
	...
	private void OpenDevEventRecordDetail(EventRecordEntry entry, int returnPage)
	{
		...
		MultiSelectionInquiryData val = new MultiSelectionInquiryData("事件详情", BuildDevEventRecordCompactDetailText(entry), list, true, 0, 1, "进入", "返回", (Action<List<InquiryElement>>)delegate(List<InquiryElement> selected)
		{
			if (selected == null || selected.Count == 0)
			{
				OpenDevEventViewerMenu(returnPage);
	...
	private string BuildDevEventMaterialDetailText(EventMaterialReference material)
	{
		...
		AppendDevNpcActionField(stringBuilder, "来源StableKey", string.Join(" | ", from x in material.SourceStableKeys ?? new List<string>()
			where !string.IsNullOrWhiteSpace(x)
			select x.Trim()));
		AppendDevNpcActionField(stringBuilder, "来源ActionKind", string.Join(" | ", from x in material.SourceActionKinds ?? new List<string>()
			where !string.IsNullOrWhiteSpace(x)
			select x.Trim()));
	...
	private void ApplyWeeklyPromptMaterialAggregation(WeeklyEventMaterialPreviewGroup group)
	{
		...
		List<EventMaterialReference> source = (from x in OrderWeeklyPreviewMaterials(@group.Materials)
			where x != null
			select x).Select(CloneEventMaterialReference).ToList();
		List<EventMaterialReference> list = source.Where((EventMaterialReference x) => !IsWeeklyPromptAggregatableMaterial(x)).ToList();
		List<EventMaterialReference> list2 = source.Where(IsWeeklyPromptAggregatableMaterial).ToList();
		if (list2.Count == 0)
		{
		...
		{
			string key = ResolveWeeklyPromptAggregateCategory(item);
			string key2 = BuildWeeklyPromptAggregateEventKey(item);
			if (!dictionary.TryGetValue(key, out var value))
			{
				value = (dictionary[key] = new Dictionary<string, List<EventMaterialReference>>(StringComparer.OrdinalIgnoreCase));
			}
			if (!value.TryGetValue(key2, out var value2))
			{
				value2 = (value[key2] = new List<EventMaterialReference>());
			}
			value2.Add(item);
		}
		...
		group.Materials = OrderWeeklyPreviewMaterials(list.Concat(list4).ToList()).ToList();
	}
	...
	private static bool IsWeeklyPromptAggregatableMaterial(EventMaterialReference material)
	{
		...
		string a = (material.MaterialType ?? "").Trim();
		return string.Equals(a, "npc_recent_action", StringComparison.OrdinalIgnoreCase) || string.Equals(a, "npc_major_action", StringComparison.OrdinalIgnoreCase);
	}

	private static List<string> GetWeeklyPromptAggregateCategoryOrder()
	{
		return new List<string> { "strategic_shift", "decision", "siege", "settlement_change", "captivity", "release", "army", "battle", "movement", "other" };
	}

	private static string ResolveWeeklyPromptAggregateCategory(EventMaterialReference material)
	{
		switch ((material?.ActionKind ?? "").Trim().ToLowerInvariant())
		{
	...
	private EventMaterialReference BuildWeeklyPromptAggregateCategoryMaterial(WeeklyEventMaterialPreviewGroup group, string category, Dictionary<string, List<EventMaterialReference>> eventBuckets)
	{
		...
		List<List<EventMaterialReference>> list = (from x in eventBuckets.Values
			where x != null && x.Count > 0
			orderby x.Min((EventMaterialReference y) => y?.ActionDay ?? int.MaxValue), x.Min((EventMaterialReference y) => y?.ActionSequence ?? int.MaxValue)
			select x).ThenBy((List<EventMaterialReference> x) => x.FirstOrDefault((EventMaterialReference y) => y != null)?.Label ?? "", StringComparer.OrdinalIgnoreCase).ToList();
		if (list.Count == 0)
		{
			return null;
		}
		string weeklyPromptAggregateCategoryLabel = GetWeeklyPromptAggregateCategoryLabel(category);
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.AppendLine("[" + weeklyPromptAggregateCategoryLabel + "]");
		foreach (List<EventMaterialReference> item in list)
		{
			string text = BuildWeeklyPromptAggregateEventLine(item);
			if (!string.IsNullOrWhiteSpace(text))
			{
		...
		EventMaterialReference eventMaterialReference = new EventMaterialReference
		{
			...
			Label = weeklyPromptAggregateCategoryLabel,
			SnapshotText = text2,
	...
	private string BuildWeeklyPromptAggregateEventLine(List<EventMaterialReference> materials)
	{
		...
		return ResolveWeeklyPromptAggregateCategory(materials[0]) switch
		{
			"strategic_shift" => BuildWeeklyPromptAggregateClanChangeLine(materials), 
			"settlement_change" => BuildWeeklyPromptAggregateSettlementOwnerChangeLine(materials), 
			"captivity" => BuildWeeklyPromptAggregatePrisonerLine(materials, released: false), 
			"release" => BuildWeeklyPromptAggregatePrisonerLine(materials, released: true), 
			"decision" => BuildWeeklyPromptAggregateDecisionLine(materials), 
			"siege" => BuildWeeklyPromptAggregateSiegeLine(materials), 
			"movement" => BuildWeeklyPromptAggregateMovementLine(materials), 
			_ => BuildWeeklyPromptAggregateGenericLine(materials), 
		};
	}
	...
	private string BuildWeeklyPromptAggregateGenericLine(List<EventMaterialReference> materials)
	{
		...
		AppendWeeklyPromptAggregateField(list, "人物", ResolveHeroNames(CollectMaterialIds(materials, (EventMaterialReference x) => new string[3] { x.ActorHeroId, x.TargetHeroId, x.HeroId }, (EventMaterialReference x) => x.RelatedHeroIds)));
		AppendWeeklyPromptAggregateField(list, "家族", ResolveClanNames(CollectMaterialIds(materials, (EventMaterialReference x) => new string[4] { x.ActorClanId, x.TargetClanId, x.SettlementOwnerClanId, x.PreviousSettlementOwnerClanId }, (EventMaterialReference x) => x.RelatedClanIds)));
		AppendWeeklyPromptAggregateField(list, "王国", ResolveKingdomNames(CollectMaterialIds(materials, (EventMaterialReference x) => new string[5] { x.ActorKingdomId, x.TargetKingdomId, x.KingdomId, x.SettlementOwnerKingdomId, x.PreviousSettlementOwnerKingdomId }, (EventMaterialReference x) => x.RelatedKingdomIds)));
	...
	private string BuildWeeklyPromptAggregateDecisionLine(List<EventMaterialReference> materials)
	{
		...
		AppendWeeklyPromptAggregateField(list, "相关家族", ResolveClanNames(CollectMaterialIds(materials, (EventMaterialReference x) => new string[3] { x.ActorClanId, x.TargetClanId, x.SettlementOwnerClanId }, (EventMaterialReference x) => x.RelatedClanIds)));
		AppendWeeklyPromptAggregateField(list, "相关王国", ResolveKingdomNames(CollectMaterialIds(materials, (EventMaterialReference x) => new string[4] { x.ActorKingdomId, x.TargetKingdomId, x.KingdomId, x.SettlementOwnerKingdomId }, (EventMaterialReference x) => x.RelatedKingdomIds)));
		AppendWeeklyPromptAggregateField(list, "涉及地点", ResolveSettlementNames(CollectMaterialIds(materials, (EventMaterialReference x) => new string[1] { x.SettlementId }, null)));
		return string.Join("|", list);
	}
	...
	private string BuildWeeklyPromptAggregateSiegeLine(List<EventMaterialReference> materials)
	{
		...
		list.Add("事件=" + BuildWeeklyPromptAggregateActionLabel(materials));
		AppendWeeklyPromptAggregateField(list, "地点", ResolveSettlementNames(CollectMaterialIds(materials, (EventMaterialReference x) => new string[1] { x.SettlementId }, null)));
		AppendWeeklyPromptAggregateField(list, "参与人物", ResolveHeroNames(CollectMaterialIds(materials, (EventMaterialReference x) => new string[2] { x.ActorHeroId, x.HeroId }, (EventMaterialReference x) => x.RelatedHeroIds)));
		AppendWeeklyPromptAggregateField(list, "参与家族", ResolveClanNames(CollectMaterialIds(materials, (EventMaterialReference x) => new string[1] { x.ActorClanId }, (EventMaterialReference x) => x.RelatedClanIds)));
		AppendWeeklyPromptAggregateField(list, "参与王国", ResolveKingdomNames(CollectMaterialIds(materials, (EventMaterialReference x) => new string[2] { x.ActorKingdomId, x.KingdomId }, (EventMaterialReference x) => x.RelatedKingdomIds)));
		...
		List<string> values = (from x in materials.Where((EventMaterialReference x) => x != null && x.ActionKind == "siege_complete").Select(delegate(EventMaterialReference x)
			{
			...
			where !string.IsNullOrWhiteSpace(x)
			select x).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
		AppendWeeklyPromptAggregateField(list, "结果", values);
		return string.Join("|", list);
	}
	...
	private string BuildWeeklyPromptAggregatePrisonerLine(List<EventMaterialReference> materials, bool released)
	{
		...
		List<Tuple<string, string, string>> entries2 = new List<Tuple<string, string, string>>();
		foreach (EventMaterialReference item in materials.Where((EventMaterialReference x) => x != null))
		{
			...
			if (!released)
			{
				if (text == "prisoner_taken_captor")
				{
					...
					AddUniqueId(list4, item.ActorClanId);
					AddUniqueId(list5, item.TargetClanId);
					AddUniqueId(list6, item.ActorKingdomId);
					AddUniqueId(list7, item.TargetKingdomId);
					AddWeeklyPromptAggregateAffiliationEntry(entries, item.ActorHeroId, item.ActorClanId, item.ActorKingdomId);
					AddWeeklyPromptAggregateAffiliationEntry(entries2, item.TargetHeroId, item.TargetClanId, item.TargetKingdomId);
				}
				else if (text == "prisoner_taken_prisoner")
				{
					AddUniqueId(list2, item.TargetHeroId);
					AddUniqueId(list3, item.ActorHeroId);
					AddUniqueId(list4, item.TargetClanId);
					AddUniqueId(list5, item.ActorClanId);
					AddUniqueId(list6, item.TargetKingdomId);
					AddUniqueId(list7, item.ActorKingdomId);
					AddWeeklyPromptAggregateAffiliationEntry(entries, item.TargetHeroId, item.TargetClanId, item.TargetKingdomId);
					AddWeeklyPromptAggregateAffiliationEntry(entries2, item.ActorHeroId, item.ActorClanId, item.ActorKingdomId);
				}
			}
		...
		AppendWeeklyPromptAggregateField(list, released ? "原囚禁方归属" : "俘获者归属", BuildWeeklyPromptAggregateAffiliationValues(entries));
		AppendWeeklyPromptAggregateField(list, released ? "获释者归属" : "被俘者归属", BuildWeeklyPromptAggregateAffiliationValues(entries2));
		...
		AppendWeeklyPromptAggregateField(list, released ? "原囚禁方家族" : "俘获者家族", ResolveClanNames(list4));
		AppendWeeklyPromptAggregateField(list, released ? "获释者家族" : "被俘者家族", ResolveClanNames(list5));
		AppendWeeklyPromptAggregateField(list, released ? "原囚禁方王国" : "俘获者王国", ResolveKingdomNames(list6));
		AppendWeeklyPromptAggregateField(list, released ? "获释者王国" : "被俘者王国", ResolveKingdomNames(list7));
		AppendWeeklyPromptAggregateField(list, "地点", ResolveSettlementNames(CollectMaterialIds(materials, (EventMaterialReference x) => new string[1] { x.SettlementId }, null)));
		if (released)
		{
			List<string> values = (from x in materials
				select ParseWeeklyPromptReleaseDetail(x?.ActionStableKey) into x
				where !string.IsNullOrWhiteSpace(x)
				select x).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
			AppendWeeklyPromptAggregateField(list, "方式", values);
		}
		return string.Join("|", list);
	}
	...
	private string BuildWeeklyPromptAggregateSettlementOwnerChangeLine(List<EventMaterialReference> materials)
	{
		...
		foreach (EventMaterialReference item in materials.Where((EventMaterialReference x) => x != null))
		{
			switch ((item.ActionKind ?? "").Trim().ToLowerInvariant())
			{
			case "settlement_owner_changed_gain":
				AddUniqueId(list2, item.ActorHeroId);
				AddUniqueId(list3, item.TargetHeroId);
		...
		List<string> values = (from x in materials
			select ParseWeeklyPromptSettlementChangeDetail(x?.ActionStableKey) into x
			where !string.IsNullOrWhiteSpace(x)
			select x).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
		AppendWeeklyPromptAggregateField(list, "方式", values);
		return string.Join("|", list);
	...
	private string BuildWeeklyPromptAggregateClanChangeLine(List<EventMaterialReference> materials)
	{
		List<string> list = new List<string>();
		list.Add("事件=家族归属变更");
		AppendWeeklyPromptAggregateField(list, "家族", ResolveClanNames(CollectMaterialIds(materials, (EventMaterialReference x) => new string[1] { x.ActorClanId }, (EventMaterialReference x) => x.RelatedClanIds)));
		AppendWeeklyPromptAggregateField(list, "成员", ResolveHeroNames(CollectMaterialIds(materials, (EventMaterialReference x) => new string[2] { x.ActorHeroId, x.HeroId }, (EventMaterialReference x) => x.RelatedHeroIds)));
		List<string> kingdomIds = (from x in materials
			select ParseWeeklyPromptClanChangeDetail(x?.ActionStableKey)?.oldKingdomId into x
			where !string.IsNullOrWhiteSpace(x)
			select x).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
		List<string> kingdomIds2 = (from x in materials
			select ParseWeeklyPromptClanChangeDetail(x?.ActionStableKey)?.newKingdomId into x
			where !string.IsNullOrWhiteSpace(x)
			select x).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
		List<string> values = (from x in materials
			select ParseWeeklyPromptClanChangeDetail(x?.ActionStableKey)?.detailLabel into x
			where !string.IsNullOrWhiteSpace(x)
			select x).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
		AppendWeeklyPromptAggregateField(list, "原王国", ResolveKingdomNames(kingdomIds));
		AppendWeeklyPromptAggregateField(list, "新王国", ResolveKingdomNames(kingdomIds2));
		AppendWeeklyPromptAggregateField(list, "方式", values);
		return string.Join("|", list);
	}
	...
	{
		List<string> list = (from x in materials ?? new List<EventMaterialReference>()
			where x != null
			select TranslateNpcActionKindForPrompt(x.ActionKind) into x
			where !string.IsNullOrWhiteSpace(x)
			select x).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
		if (list.Count > 0)
		{
	...
	private static void AppendWeeklyPromptAggregateField(List<string> fields, string name, List<string> values)
	{
		...
		{
			List<string> list = (from x in values
				where !string.IsNullOrWhiteSpace(x)
	...
	private static List<string> BuildWeeklyPromptAggregateHeroAffiliationValues(List<EventMaterialReference> materials)
	{
		...
		foreach (EventMaterialReference item in materials ?? new List<EventMaterialReference>())
		{
			...
			AddWeeklyPromptAggregateAffiliationEntry(entries, item.ActorHeroId, item.ActorClanId, item.ActorKingdomId);
			AddWeeklyPromptAggregateAffiliationEntry(entries, item.TargetHeroId, item.TargetClanId, item.TargetKingdomId);
			AddWeeklyPromptAggregateAffiliationEntry(entries, item.HeroId, "", item.KingdomId);
			foreach (string item2 in item.RelatedHeroIds ?? new List<string>())
			{
				AddWeeklyPromptAggregateAffiliationEntry(entries, item2, "", "");
			}
		}
		return BuildWeeklyPromptAggregateAffiliationValues(entries);
	}
	...
	private static List<string> BuildWeeklyPromptAggregateAffiliationValues(IEnumerable<Tuple<string, string, string>> entries)
	{
		...
		foreach (Tuple<string, string, string> item in entries ?? Enumerable.Empty<Tuple<string, string, string>>())
		{
			...
			if (string.IsNullOrWhiteSpace(item?.Item2))
			{
				...
				{
					Clan clan = val.Clan;
					obj = ((clan != null) ? ((MBObjectBase)clan).StringId : null);
				}
				if (obj == null)
				{
					obj = "";
			...
			AddUniqueId(value, id);
			AddUniqueId(value2, id2);
		}
		return (from x in list.Select(delegate(string heroId)
			{
				string text2 = ResolveHeroDisplay(heroId);
				...
				return (list4.Count == 0) ? text2 : (text2 + "(" + string.Join(";", list4) + ")");
			})
			where !string.IsNullOrWhiteSpace(x)
			select x).ToList();
	}
	...
	private static void AddWeeklyPromptAggregateAffiliationEntry(List<Tuple<string, string, string>> entries, string heroId, string clanId, string kingdomId)
	{
		...
		if (!string.IsNullOrWhiteSpace(text))
		{
			entries?.Add(Tuple.Create(text, (clanId ?? "").Trim(), (kingdomId ?? "").Trim()));
		}
	}
	...
	{
		List<string> list = new List<string>();
		foreach (EventMaterialReference item3 in materials ?? new List<EventMaterialReference>())
		{
			if (item3 == null)
			{
				continue;
			}
			string text = (item3.SnapshotText ?? item3.Label ?? "").Trim();
			if (((item3.ActionStableKey ?? "").IndexOf("RaidSettlement", StringComparison.OrdinalIgnoreCase) >= 0 || (item3.ActionKind ?? "").Trim().Equals("daily_behavior", StringComparison.OrdinalIgnoreCase)) && !string.IsNullOrWhiteSpace(item3.SettlementId) && text.IndexOf("掠", StringComparison.OrdinalIgnoreCase) >= 0)
			{
				string text2 = BuildWeeklyPromptAggregateSingleAffiliationValue(item3.ActorHeroId, item3.ActorClanId, item3.ActorKingdomId);
				string text3 = ResolveSettlementDisplay(item3.SettlementId);
				List<string> list2 = new List<string>();
				string text4 = ResolveClanDisplay(item3.SettlementOwnerClanId);
				string text5 = ResolveKingdomDisplay(item3.SettlementOwnerKingdomId);
				if (!string.IsNullOrWhiteSpace(text4))
				{
					list2.Add("家族=" + text4);
				}
				...
				string item = text7 + "正在掠夺" + text8 + text6;
			...
			string text9 = ResolveHeroDisplay((!string.IsNullOrWhiteSpace(item3.ActorHeroId)) ? item3.ActorHeroId : ((!string.IsNullOrWhiteSpace(item3.HeroId)) ? item3.HeroId : item3.TargetHeroId));
			string text10 = text;
			if (!string.IsNullOrWhiteSpace(text10))
			{
				string item2 = (string.IsNullOrWhiteSpace(text9) ? text10 : (text9 + "：" + text10));
	...
	{
		return (from x in (heroIds ?? Enumerable.Empty<string>()).Select(ResolveHeroDisplay)
			where !string.IsNullOrWhiteSpace(x)
			select x).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
	}
	...
	{
		return (from x in (clanIds ?? Enumerable.Empty<string>()).Select(ResolveClanDisplay)
			where !string.IsNullOrWhiteSpace(x)
			select x).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
	}
	...
	{
		return (from x in (kingdomIds ?? Enumerable.Empty<string>()).Select(ResolveKingdomDisplay)
			where !string.IsNullOrWhiteSpace(x)
			select x).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
	}
	...
	{
		return (from x in (settlementIds ?? Enumerable.Empty<string>()).Select(ResolveSettlementDisplay)
			where !string.IsNullOrWhiteSpace(x)
			select x).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
	}
	...
	private static string GetWeeklyPromptAggregateCategoryLabel(string category)
	{
		...
		{
			"strategic_shift" => "家族与王国归属", 
			"decision" => "王国决议", 
			"siege" => "围城与守城", 
			"settlement_change" => "定居点易主", 
			"captivity" => "人物被俘", 
			"release" => "人物获释", 
			"army" => "军团行动", 
			"battle" => "战场交锋", 
	...
	private static string ParseWeeklyPromptSettlementChangeDetail(string stableKey)
	{
		...
		return (array[array.Length - 1] ?? "").Trim() switch
		{
			"BySiege" => "围城", 
			"ByBarter" => "易物", 
			"ByLeaveFaction" => "脱离王国", 
			...
			"ByGift" => "赠与", 
			"ByRebellion" => "叛乱", 
			"ByClanDestruction" => "家族覆灭", 
			_ => (array[array.Length - 1] ?? "").Trim(), 
		};
	}
	...
	private static string ParseWeeklyPromptReleaseDetail(string stableKey)
	{
		...
		{
			"Ransom" => "通过赎金获释", 
			"ReleasedByChoice" => "被主动释放", 
			"ReleasedAfterPeace" => "因议和获释", 
			"ReleasedAfterEscape" => "成功逃脱", 
			"ReleasedAfterBattle" => "战后获释", 
			"ReleasedByCompensation" => "补偿后获释", 
			"Death" => "囚禁中死亡", 
			_ => (array[array.Length - 1] ?? "").Trim(), 
		};
	}
	...
	private static string TranslateWeeklyPromptClanChangeDetail(string detail)
	{
		...
		{
			"JoinAsMercenary" => "佣兵加入", 
			"JoinKingdom" => "正式加入", 
			"JoinKingdomByDefection" => "叛逃改投", 
			"LeaveKingdom" => "脱离王国", 
			"LeaveWithRebellion" => "脱离并叛乱", 
			"LeaveAsMercenary" => "结束佣兵服务", 
			"CreateKingdom" => "建立新王国", 
			"LeaveByKingdomDestruction" => "原王国覆灭", 
			"LeaveByClanDestruction" => "家族覆灭", 
			_ => (detail ?? "").Trim(), 
		};
	}
	...
	{
		return (from x in (batch?.Groups ?? new List<WeeklyEventMaterialPreviewGroup>()).Select(BuildWeeklyReportGroupReportId)
			where !string.IsNullOrWhiteSpace(x)
			select x).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
	}
	...
	private static List<WeeklyReportBatchRequest> BuildWeeklyReportBatchRequests(List<WeeklyEventMaterialPreviewGroup> groups, int weekIndex, int startDay, int endDay)
	{
		...
		int num = Math.Max(1, GetWeeklyReportBatchSize());
		for (int num2 = 0; num2 < list3.Count; num2 += num)
		{
			list.Add(new WeeklyReportBatchRequest
			{
				WeekIndex = weekIndex,
				StartDay = startDay,
				EndDay = endDay,
				Groups = list3.Skip(num2).Take(num).ToList()
			});
		}
		return list;
	}
	...
	{
		List<string> list = (from x in (batch?.Groups ?? new List<WeeklyEventMaterialPreviewGroup>()).Select(BuildWeeklyReportGroupDisplayLabel)
			where !string.IsNullOrWhiteSpace(x)
			select x).ToList();
	...
	{
		List<string> values = BuildWeeklyBatchExpectedReportIds(batch);
		return (batch?.WeekIndex ?? 0) + "|" + string.Join("|", values);
	}
	...
	private List<string> SelectWeeklyShortReportKingdomIds(string npcKingdomId, bool excludeNpcKingdom)
	{
		...
		List<string> list = (from x in GetKingdomIdsByPlayerProximity(from x in GetDevEditableKingdoms().Where(IsKingdomEligibleForWeeklyReport)
				select (x != null) ? ((MBObjectBase)x).StringId : null)
			where !string.IsNullOrWhiteSpace(x)
			select x.Trim()).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
		if (list.Count == 0)
		{
			list = (from x in GetDevEditableKingdoms().Where(IsKingdomEligibleForWeeklyReport)
	...
	private async Task<WeeklyReportBatchRequestResult> GenerateWeeklyReportBatchWithRetriesAsync(WeeklyReportBatchRequest batch, int maxAttempts)
	{
		...
		{
			ApiCallResult apiCallResult = await CallUniversalApiDetailed(text, text2);
			Logger.LogEventPromptExchange(replyText: weeklyReportBatchRequestResult.RawResponse = (apiCallResult.Success ? (apiCallResult.Content ?? "") : ("閿欒\ue1e4: " + (apiCallResult.ErrorMessage ?? "鏈\ue046煡閿欒\ue1e4"))), targetLabel: text4 + " [灏濊瘯 " + i + "/" + maxAttempts + "]", requestText: text3);
			...
			{
				if (!TryParseWeeklyBatchResponse(apiCallResult.Content, batch, out var blocks, out var missingReportIds, out var failureReason))
				{
					weeklyReportBatchRequestResult.Success = false;
	...
	private async Task<WeeklyReportBatchExecutionResult> ExecuteWeeklyReportBatchAsync(WeeklyReportBatchRequest batch, int batchIndex, int maxAttempts)
	{
		...
		catch (Exception ex)
		{
			...
			{
				Success = false,
				FailureReason = "Batch execution crashed: " + ex2.Message,
				AttemptsUsed = 1,
				Blocks = new List<WeeklyReportBatchBlockResult>(),
				MissingReportIds = (from x in (batch?.Groups ?? new List<WeeklyEventMaterialPreviewGroup>()).Select(BuildWeeklyReportGroupReportId)
					where !string.IsNullOrWhiteSpace(x)
					select x).Distinct(StringComparer.OrdinalIgnoreCase).ToList()
			};
		}
	...
	private async Task RetryBlockedWeeklyReportsAsync(WeeklyReportRetryContext context, int retryVersion)
	{
		try
		{
			WeeklyReportGenerationResult weeklyReportGenerationResult = await GenerateWeeklyReportsBatchedAsyncInternal(context.Groups, context.WeekIndex, context.StartDay, context.EndDay, context.DisplayLabel, context.OpenViewerWhenDone, queueBlockingPopupOnFatalFailure: false, context.IsAutoGeneration);
	...
	private string BuildWeeklyBatchReportUserPrompt(WeeklyReportBatchRequest batch)
	{
		...
		stringBuilder.AppendLine("week_index=" + (batch?.WeekIndex ?? 0));
		stringBuilder.AppendLine("day_range=" + (batch?.StartDay ?? 0) + "-" + (batch?.EndDay ?? 0));
		stringBuilder.AppendLine("block_count=" + list.Count);
		foreach (WeeklyEventMaterialPreviewGroup item in list)
		{
			stringBuilder.AppendLine();
			stringBuilder.AppendLine(BuildWeeklyBatchPromptInputBlock(item, batch?.WeekIndex ?? 0));
		}
		return stringBuilder.ToString().TrimEnd();
	}
	...
	private string BuildWeeklyBatchPromptInputBlock(WeeklyEventMaterialPreviewGroup group, int weekIndex)
	{
		...
		if (!flag)
		{
			...
			string text = BuildWeeklyReportCurrentKingdomStabilityTierText(group);
			if (!string.IsNullOrWhiteSpace(text))
			{
				stringBuilder.AppendLine("stability=" + text.Replace("当前王国稳定度评级：", "").Trim());
			}
		}
	...
	private static string BuildWeeklyBatchPromptPreviewText(WeeklyReportBatchRequest batch, string systemPrompt, string userPrompt)
	{
		StringBuilder stringBuilder = new StringBuilder();
		List<WeeklyEventMaterialPreviewGroup> list = (batch?.Groups ?? new List<WeeklyEventMaterialPreviewGroup>()).Where((WeeklyEventMaterialPreviewGroup x) => x != null).ToList();
		AppendDevNpcActionField(stringBuilder, "批次周数", (batch?.WeekIndex ?? 0).ToString());
		AppendDevNpcActionField(stringBuilder, "取材区间", (batch?.StartDay ?? 0) + "-" + (batch?.EndDay ?? 0));
		AppendDevNpcActionField(stringBuilder, "批次块数", list.Count.ToString());
		AppendDevNpcActionField(stringBuilder, "批次对象", string.Join(" | ", from x in list.Select(BuildWeeklyReportGroupReportId)
			where !string.IsNullOrWhiteSpace(x)
			select x));
		AppendDevNpcActionField(stringBuilder, "MaxTokens", 5000.ToString());
		stringBuilder.AppendLine();
	...
	private static bool TryParseWeeklyBatchResponse(string rawResponse, WeeklyReportBatchRequest batch, out List<WeeklyReportBatchBlockResult> blocks, out List<string> missingReportIds, out string failureReason)
	{
		...
		Dictionary<string, WeeklyEventMaterialPreviewGroup> groupMap = BuildWeeklyReportGroupMap(batch?.Groups);
		...
		{
			string rawBlockBody = (item.Groups["body"]?.Value ?? "").Trim();
			if (!TryParseWeeklyBatchBlock(rawBlockBody, batch, groupMap, out var block))
			{
				block = block ?? new WeeklyReportBatchBlockResult();
			}
			if (!string.IsNullOrWhiteSpace(block.ReportId) && !dictionary.ContainsKey(block.ReportId))
			{
				dictionary[block.ReportId] = block;
			}
			blocks.Add(block);
		}
		...
		return blocks.Any((WeeklyReportBatchBlockResult x) => x?.Parsed ?? false);
	}
	...
	private static bool TryParseWeeklyBatchBlock(string rawBlockBody, WeeklyReportBatchRequest batch, Dictionary<string, WeeklyEventMaterialPreviewGroup> groupMap, out WeeklyReportBatchBlockResult block)
	{
		...
		{
			string b = (value.KingdomId ?? "").Trim();
			if (string.IsNullOrWhiteSpace((block.KingdomId ?? "").Trim()) || !string.Equals((block.KingdomId ?? "").Trim(), b, StringComparison.OrdinalIgnoreCase))
			{
				block.FailureReason = "kingdom_id 与输入批次不一致。";
				return false;
			}
		}
		string input = Regex.Replace(text, "^report_id=.*?$", "", RegexOptions.IgnoreCase | RegexOptions.Multiline).Trim();
		input = Regex.Replace(input, "^kind=.*?$", "", RegexOptions.IgnoreCase | RegexOptions.Multiline).Trim();
		input = Regex.Replace(input, "^kingdom_id=.*?$", "", RegexOptions.IgnoreCase | RegexOptions.Multiline).Trim();
		if (!TryParseWeeklyReportResponse(input, value, batch?.WeekIndex ?? 0, out var title, out var shortSummary, out var report, out var tagText))
		{
			block.FailureReason = BuildWeeklyReportFailureReason(input, parseFailed: true);
			return false;
		}
		block.Title = title;
	...
	private static string ExtractWeeklyBatchHeaderValue(string text, string key)
	{
		...
		Match match = Regex.Match(text, "^" + Regex.Escape(key.Trim()) + "=(?<value>.*)$", RegexOptions.IgnoreCase | RegexOptions.Multiline);
	...
	private void UpsertWeeklyReportEventRecord(WeeklyEventMaterialPreviewGroup group, int weekIndex, string title, string shortSummary, string report, string tagText, string promptText)
	{
		...
		eventRecordEntry.Materials = (from x in OrderWeeklyPreviewMaterials(@group?.Materials)
			select (x == null) ? null : new EventMaterialReference
			{
				...
				RelatedHeroIds = new List<string>(from y in x.RelatedHeroIds ?? new List<string>()
					where !string.IsNullOrWhiteSpace(y)
					select y.Trim()),
				RelatedClanIds = new List<string>(from y in x.RelatedClanIds ?? new List<string>()
					where !string.IsNullOrWhiteSpace(y)
					select y.Trim()),
				RelatedKingdomIds = new List<string>(from y in x.RelatedKingdomIds ?? new List<string>()
					where !string.IsNullOrWhiteSpace(y)
					select y.Trim()),
				SourceStableKeys = new List<string>(from y in x.SourceStableKeys ?? new List<string>()
					where !string.IsNullOrWhiteSpace(y)
					select y.Trim()),
				SourceActionKinds = new List<string>(from y in x.SourceActionKinds ?? new List<string>()
					where !string.IsNullOrWhiteSpace(y)
					select y.Trim()),
	...
	private void TryAddPreviewActionMaterial(List<EventMaterialReference> materials, Hero hero, NpcActionEntry entry, bool recentOnly)
	{
		...
		EventMaterialReference eventMaterialReference2 = new EventMaterialReference
		{
			...
			RelatedHeroIds = new List<string>(from x in entry.RelatedHeroIds ?? new List<string>()
				where !string.IsNullOrWhiteSpace(x)
				select x.Trim()),
			RelatedClanIds = new List<string>(from x in entry.RelatedClanIds ?? new List<string>()
				where !string.IsNullOrWhiteSpace(x)
				select x.Trim()),
			RelatedKingdomIds = new List<string>(from x in entry.RelatedKingdomIds ?? new List<string>()
				where !string.IsNullOrWhiteSpace(x)
				select x.Trim()),
	...
	private void OpenDevWeeklyReportPromptPreviewMenu()
	{
		...
		List<InquiryElement> list = new List<InquiryElement>();
		list.Add(new InquiryElement((object)"back", "返回", (ImageIdentifier)null));
		list.Add(new InquiryElement((object)"single", "查看单组 Prompt", (ImageIdentifier)null));
		list.Add(new InquiryElement((object)"batch", "查看 Batch Prompt/Response", (ImageIdentifier)null));
		MultiSelectionInquiryData val = new MultiSelectionInquiryData("本周周报 Prompt 预览", "选择要查看的调试视图。单组视图用于逐王国核对，Batch 视图用于查看实际批量请求和最近一次返回。", list, true, 0, 1, "进入", "返回", (Action<List<InquiryElement>>)delegate(List<InquiryElement> selected)
		{
			if (selected == null || selected.Count == 0)
			{
				OpenDevEventEditorMenu();
	...
	private void OpenDevWeeklyBatchPromptPreviewMenu()
	{
		...
		List<WeeklyReportBatchRequest> list = BuildWeeklyReportBatchRequests(OrderWeeklyReportGenerationGroups(BuildWeeklyEventMaterialPreviewGroups()), weekIndex, startDay, currentGameDayIndexSafe);
		List<InquiryElement> list2 = new List<InquiryElement>();
		list2.Add(new InquiryElement((object)"back", "返回", (ImageIdentifier)null));
		foreach (WeeklyReportBatchRequest item in list)
		{
			DevWeeklyReportBatchPreviewEntry devWeeklyReportBatchPreviewEntry = FindLatestWeeklyReportBatchDevPreview(item);
			string text = ((devWeeklyReportBatchPreviewEntry == null) ? "未执行" : (devWeeklyReportBatchPreviewEntry.Success ? "有响应" : "失败响应"));
			int count = (item?.Groups ?? new List<WeeklyEventMaterialPreviewGroup>()).Count;
			list2.Add(new InquiryElement((object)item, BuildWeeklyReportBatchDisplayLabel(item) + " [" + count + "块 | " + text + "]", (ImageIdentifier)null));
		}
		MultiSelectionInquiryData val = new MultiSelectionInquiryData("本周周报 Batch Prompt/Response", BuildWeeklyBatchPromptPreviewMenuDescription(list), list2, true, 0, 1, "查看详情", "返回", (Action<List<InquiryElement>>)delegate(List<InquiryElement> selected)
		{
			if (selected == null || selected.Count == 0)
			{
				OpenDevWeeklyReportPromptPreviewMenu();
			}
			else if (selected[0].Identifier is string text2 && text2 == "back")
			{
				OpenDevWeeklyReportPromptPreviewMenu();
			}
			else if (selected[0].Identifier is WeeklyReportBatchRequest batch)
			{
				OpenDevWeeklyBatchPromptDetail(batch);
			}
			else
			{
				OpenDevWeeklyBatchPromptPreviewMenu();
	...
	private string BuildWeeklyBatchPromptPreviewMenuDescription(List<WeeklyReportBatchRequest> batches)
	{
		...
		stringBuilder.AppendLine(" ");
		stringBuilder.AppendLine("批次数量：" + (batches?.Count ?? 0));
	...
	private void OpenDevWeeklyBatchPromptDetail(WeeklyReportBatchRequest batch)
	{
		...
		string systemPrompt = BuildWeeklyBatchReportSystemPrompt();
		string userPrompt = BuildWeeklyBatchReportUserPrompt(batch);
		string value = BuildWeeklyBatchPromptPreviewText(batch, systemPrompt, userPrompt);
		DevWeeklyReportBatchPreviewEntry devWeeklyReportBatchPreviewEntry = FindLatestWeeklyReportBatchDevPreview(batch);
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.AppendLine(value);
		stringBuilder.AppendLine();
		...
		{
			stringBuilder.AppendLine("执行结果：" + (devWeeklyReportBatchPreviewEntry.Success ? "成功" : "失败"));
			stringBuilder.AppendLine("尝试次数：" + devWeeklyReportBatchPreviewEntry.AttemptsUsed);
			if (!string.IsNullOrWhiteSpace(devWeeklyReportBatchPreviewEntry.FailureReason))
			{
				stringBuilder.AppendLine("失败原因：" + devWeeklyReportBatchPreviewEntry.FailureReason);
			}
			stringBuilder.AppendLine();
			stringBuilder.AppendLine(string.IsNullOrWhiteSpace(devWeeklyReportBatchPreviewEntry.ResponsePreview) ? "响应原文为空。" : devWeeklyReportBatchPreviewEntry.ResponsePreview.Trim());
		}
		InformationManager.ShowInquiry(new InquiryData("Batch Prompt/Response 详情", stringBuilder.ToString().TrimEnd(), true, false, "返回 Batch 列表", "", (Action)delegate
		{
			OpenDevWeeklyBatchPromptPreviewMenu();
		}, (Action)null, "", 0f, (Action)null, (Func<ValueTuple<bool, string>>)null, (Func<ValueTuple<bool, string>>)null), false, false);
	}
	...
	private void ConfirmGenerateDevWeeklyReports()
	{
		...
		int startDay = Math.Max(0, currentGameDayIndexSafe - currentGameDayIndexSafe % 7);
		int weekIndex = Math.Max(1, currentGameDayIndexSafe / 7 + 1);
		int count = BuildWeeklyReportBatchRequests(list, weekIndex, startDay, currentGameDayIndexSafe).Count;
		List<string> kingdomIdsByPlayerProximity = GetKingdomIdsByPlayerProximity(from x in list
			where string.Equals((x.GroupKind ?? "").Trim(), "kingdom", StringComparison.OrdinalIgnoreCase)
			select x.KingdomId);
	...
	private async Task<WeeklyReportGenerationResult> GenerateWeeklyReportsMinuteBurstAsyncInternal(List<WeeklyEventMaterialPreviewGroup> list, int weekIndex, int startDay, int endDay, string displayLabel, bool openViewerWhenDone, bool queueBlockingPopupOnFatalFailure, bool isAutoGeneration)
	{
		...
		{
			List<WeeklyReportBatchRequest> wave = (from x in batches.Skip(i).Take(burstSize)
				where x != null && x.Groups != null && x.Groups.Count > 0
				select x).ToList();
			if (wave.Count != 0)
			{
				int waveIndex = i / burstSize + 1;
				string waveLabel = string.Join(" | ", from x in wave.Select(BuildWeeklyReportBatchDisplayLabel)
					where !string.IsNullOrWhiteSpace(x)
					select x);
		...
		foreach (WeeklyReportBatchExecutionResult execution in completed.OrderBy((WeeklyReportBatchExecutionResult x) => x.BatchIndex))
		{
			...
			{
				if (block != null && block.Parsed && !string.IsNullOrWhiteSpace(block.ReportId) && parsedReportIds.Add(block.ReportId) && groupMap.TryGetValue(block.ReportId, out var group) && group != null)
				{
