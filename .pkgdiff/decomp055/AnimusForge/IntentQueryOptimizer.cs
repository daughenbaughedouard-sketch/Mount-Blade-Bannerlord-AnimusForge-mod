...
internal static class IntentQueryOptimizer
{
	...
	internal const int MaxCombinedIntentCount = 4;
	...
	private static readonly string[] LeadingPhrases = new string[38]
	{
		...
		"我想问", "我想知道", "我想请教", "能告诉我", "能不能告诉我", "能不能说说", "可以告诉我", "可以说说", "你知道不知道", "你知道吗",
		"你知道", "你可知道", "说说", "告诉我", "帮我说说", "麻烦说说", "顺便问一下", "顺带问一下"
	};
	...
	public static List<string> OptimizeSplitIntents(IEnumerable<string> intents, int maxCount)
	{
		...
		try
		{
			...
			foreach (string item in list2)
			{
				...
				string text2 = BuildCanonicalIntent(item);
				...
				{
					if (AreNearDuplicateCanonical(text2, list3[i]))
					{
						flag = true;
						break;
					}
	...
	private static bool IsLowInformationIntent(string text)
	{
		...
		string[] lowSignalExactPhrases = LowSignalExactPhrases;
		foreach (string b in lowSignalExactPhrases)
		{
