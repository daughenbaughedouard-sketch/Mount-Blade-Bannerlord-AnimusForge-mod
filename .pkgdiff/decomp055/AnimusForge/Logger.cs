...
public static class Logger
{
	...
	static Logger()
	{
		...
		try
		{
			...
			string text = Path.Combine(basePath, "Modules", "AnimusForge", "Logs");
			if (!Directory.Exists(text))
			{
				Directory.CreateDirectory(text);
			}
			_modLogPath = Path.Combine(text, "Mod_Logic.txt");
			_gameTracePath = Path.Combine(text, "Game_Trace.txt");
			_obsLogPath = Path.Combine(text, "Observability.jsonl");
			_hitRatePath = Path.Combine(text, "HitRate_Stats.txt");
			_tokenStatsPath = Path.Combine(text, "Token_Stats.txt");
			_eventLogsPath = Path.Combine(text, "Event_Logs.txt");
			...
			{
				AppendUtf8(_modLogPath, content);
			}
			if (IsPathEnabled(_gameTracePath))
			{
				AppendUtf8(_gameTracePath, content);
			}
			if (IsPathEnabled(_hitRatePath))
			{
				AppendUtf8(_hitRatePath, content);
			}
			if (IsPathEnabled(_tokenStatsPath))
			{
				AppendUtf8(_tokenStatsPath, content);
			}
			if (IsPathEnabled(_eventLogsPath))
			{
				AppendUtf8(_eventLogsPath, content);
			}
			if (IsPathEnabled(_obsLogPath))
			{
				AppendUtf8(_obsLogPath, JsonConvert.SerializeObject((object)new Dictionary<string, object>
				{
	...
	public static void Obs(string source, string stage, Dictionary<string, object> fields = null)
	{
		try
		{
			...
			string line = JsonConvert.SerializeObject((object)dictionary);
	...
	private static bool TryGetMessageRoleAndContent(object message, out string role, out string content)
	{
		...
		try
		{
			JObject val = (JObject)((message is JObject) ? message : null);
			if (val != null)
			{
