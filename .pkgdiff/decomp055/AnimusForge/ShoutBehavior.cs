...
public class ShoutBehavior : CampaignBehaviorBase
{
	...
	private static string InjectSceneMechanismPromptSection(string prompt, string mechanismSection)
	{
		...
		int num2 = text.IndexOf("【附加规则:", num + "【附加规则:scene_mechanism_actions】".Length, StringComparison.Ordinal);
		if (num2 < 0)
		{
			return text.TrimEnd() + "\n" + text2;
		}
	...
	private string BuildSceneFollowControlPromptInstruction(NpcDataPacket speaker)
	{
		...
		return IsAgentFollowingPlayerBySceneCommand(val) ? "【当前正跟随玩家】若此人明确让你停止跟随且你同意，可在句末输出[STP]；若此人改让你去叫【可召目标】中的人，也可输出[ASS:id]，多人可写[ASS:id1,id2]；若此人改让你带路去找【可带路目标】或去见【可召目标】中的人，也可输出[GUI:id]。" : "";
	}
	...
	private static string StripLeakedPromptFragmentsForShout(string text)
	{
		...
		{
			"这是一场多人聊天", "其他人也要说话", "如果你觉得下一位还必须接话", "若你觉得在你这句之后", "如果你觉得这句之后还值得继续聊下去", "接力编号必须从【在场人物列表】", "id 必须从【在场人物列表】", "你不能选你自己", "才能让其他人发言", "如果不必继续，就不要输出任何续聊标签",
			"若不必继续，就不要输出任何续聊标签"
		};
	...
	private async Task<string> GenerateGroupConversationTurnLineAsync(NpcDataPacket speakerNpc, List<NpcDataPacket> allNpcData, Dictionary<int, Hero> resolvedHeroes, Dictionary<int, PrecomputedShoutRagContext> precomputedContexts, string playerText, string extraFact, string commonCandidatesPrompt, string sceneMechanismPromptSectionBase, List<string> patienceStatusLines, bool multiNpcScene, int minTokens, int maxTokens)
	{
		try
		{
			...
			bool hasPrecomputed = precomputedContexts?.TryGetValue(speakerNpc.AgentIndex, out precomputed) ?? false;
			...
			MyBehavior.ShoutPromptContext ctx = MyBehavior.BuildShoutPromptContextForExternal(hero, playerText, fullExtra, speakerNpc.CultureId ?? "neutral", speakerNpc.IsHero, characterObject, kingdomIdOverride, speakerNpc.AgentIndex, suppressDynamicRuleAndLore: false, hasPrecomputed && precomputed != null && precomputed.HasLoreContext, precomputed?.LoreContext);
		...
		{
			Exception ex2 = ex;
			Logger.Log("ShoutBehavior", "[GroupConversationTurn] prompt failed: " + ex2.Message);
			return "";
		}
	}
	...
	private async Task ProcessShoutConfirmedInternal(string shoutText, string extraFact, int? forcedPrimaryAgentIndex)
	{
		...
		{
			RefreshSceneConversationParticipantInteractions(capturedNpcData, 15f);
		}
		else
		{
	...
	private async Task HandleGroupResponsePerHeroIndependent(string playerText, List<NpcDataPacket> allNpcData, string sceneDesc, NpcDataPacket primaryNpc, string extraFact, Dictionary<int, PrecomputedShoutRagContext> precomputedContexts, Dictionary<int, Hero> resolvedHeroes, int conversationEpoch)
	{
		try
		{
			...
			string sceneMechanismPromptSectionBase = BuildSceneMechanismPromptSection(sceneSummonTargets, sceneGuideTargets, sceneSummonClosureInstruction);
			bool multiNpcScene = speakableCandidates.Count > 1;
			NpcDataPacket currentSpeaker = ((primaryNpc != null) ? speakableCandidates.FirstOrDefault((NpcDataPacket npcDataPacket) => npcDataPacket != null && npcDataPacket.AgentIndex == primaryNpc.AgentIndex) : null) ?? speakableCandidates.FirstOrDefault();
			bool firstTurn = true;
			int remainingTurns = ((!multiNpcScene) ? 1 : 8);
			while (currentSpeaker != null && remainingTurns-- > 0)
			{
				...
				if (firstTurn)
				{
					...
					PrecomputedShoutRagContext precomputed = null;
					bool hasPrecomputed = precomputedContexts?.TryGetValue(currentSpeaker.AgentIndex, out precomputed) ?? false;
					string loreContext = ((hasPrecomputed && precomputed != null) ? (precomputed.LoreContext ?? "") : "");
					string fullExtra = (string.IsNullOrWhiteSpace(loreContext) ? null : loreContext);
					if (!string.IsNullOrWhiteSpace(extraFact))
					{
					...
					Mission current = Mission.Current;
					Agent npcAgent = ((current == null) ? null : ((IEnumerable<Agent>)current.Agents)?.FirstOrDefault((Agent a) => a != null && a.Index == currentSpeaker.AgentIndex));
					MyBehavior.ShoutPromptContext ctx = MyBehavior.BuildShoutPromptContextForExternal(targetCharacter: (CharacterObject)/*isinst with value type is only supported in some contexts*/, kingdomIdOverride: TryGetKingdomIdOverrideFromAgent(npcAgent), targetHero: contextHero, input: playerText, extraFact: fullExtra, cultureIdOverride: cultureId, hasAnyHero: currentSpeaker.IsHero, targetAgentIndex: currentSpeaker.AgentIndex, suppressDynamicRuleAndLore: false, usePrefetchedLoreContext: hasPrecomputed && precomputed != null && precomputed.HasLoreContext, prefetchedLoreContext: precomputed?.LoreContext);
					StringBuilder local = new StringBuilder();
				...
				bool endRequested = ContainsAutoGroupEndSignal(cleaned);
				...
				if (endRequested)
				{
					...
					Mission current2 = Mission.Current;
					num = (shoutBehavior.IsAgentFollowingPlayerBySceneCommand((current2 == null) ? null : ((IEnumerable<Agent>)current2.Agents)?.FirstOrDefault((Agent a) => a != null && a.Index == currentSpeaker.AgentIndex && a.IsActive())) ? 1 : 0);
				}
				else
				{
					num = 0;
				...
				currentSpeaker = speakableCandidates.FirstOrDefault((NpcDataPacket npcDataPacket) => npcDataPacket != null && npcDataPacket.AgentIndex == resolvedRelayTargetAgentIndex);
				firstTurn = false;
			}
			if (multiNpcScene)
			{
							...
							if (CanAgentParticipateInSceneSpeech(val))
							{
								...
								else
								{
									if (!string.IsNullOrWhiteSpace(content))
									{
										...
										bool flag8 = allowPlayerDirectedActions && !escalatedToFight && TryTriggerSceneGuideAction(matchedNpc, val, sceneGuideTargets, sceneSummonTargets, ref content, out preparedRequest2);
										if (!string.IsNullOrWhiteSpace(content))
										{
											if (!IsSceneConversationEpochCurrent(requiredConversationEpoch))
											{
											...
											ScheduleSceneAutonomyRestoreAfterSpeech(matchedNpc.AgentIndex, playbackInfo);
	...
	private bool TryTriggerSceneGuideAction(NpcDataPacket npc, Agent agent, List<SceneGuidePromptTarget> guideTargets, List<SceneSummonPromptTarget> summonTargets, ref string content, out ActiveSceneGuideRequest preparedRequest)
	{
		...
		if ((!int.TryParse(match.Groups[1].Value.Trim(), out var result) || guideTargets == null || guideTargets.Count == 0) && (string.IsNullOrWhiteSpace(match.Groups[1].Value.Trim()) || !int.TryParse(match.Groups[1].Value.Trim(), out result)))
		{
			return false;
		}
	...
	private void CancelSceneGuideActionForAgent(int guideAgentIndex)
	{
		...
		{
			Mission current = Mission.Current;
			Agent val = ((current == null) ? null : ((IEnumerable<Agent>)current.Agents)?.FirstOrDefault((Agent a) => a != null && a.Index == guideAgentIndex));
			if (val != null)
			{
				StopSceneGuideEscort(val);
			}
	...
	private static EscortAgentBehavior GetSceneGuideEscortBehavior(Agent agent)
	{
		try
		{
			...
			{
				CampaignAgentComponent component = agent.GetComponent<CampaignAgentComponent>();
				obj = ((component != null) ? component.AgentNavigator : null);
			}
			AgentNavigator val = (AgentNavigator)obj;
			InterruptingBehaviorGroup val2 = ((val != null) ? val.GetBehaviorGroup<InterruptingBehaviorGroup>() : null);
			return (val2 != null) ? ((AgentBehaviorGroup)val2).GetBehavior<EscortAgentBehavior>() : null;
		}
		catch
		{
			return null;
	...
	private static bool TryStartEscortBehavior(Agent ownerAgent, UsableMachine targetMachine)
	{
		try
		{
			...
			{
				CampaignAgentComponent component = ownerAgent.GetComponent<CampaignAgentComponent>();
				obj = ((component != null) ? component.AgentNavigator : null);
			}
			AgentNavigator val = (AgentNavigator)obj;
			InterruptingBehaviorGroup val2 = ((val != null) ? val.GetBehaviorGroup<InterruptingBehaviorGroup>() : null);
			if (val2 == null || ownerAgent == null || Agent.Main == null || !Agent.Main.IsActive() || targetMachine == null)
			{
				return false;
			}
			EscortAgentBehavior val3 = ((AgentBehaviorGroup)val2).GetBehavior<EscortAgentBehavior>() ?? ((AgentBehaviorGroup)val2).AddBehavior<EscortAgentBehavior>();
			((AgentBehaviorGroup)val2).SetScriptedBehavior<EscortAgentBehavior>();
			val3.Initialize(Agent.Main, targetMachine, (OnTargetReachedDelegate)null);
			return true;
		}
		catch
		{
	...
	private bool TryStartSceneGuideEscort(ActiveSceneGuideRequest request, Agent guideAgent)
	{
		...
		try
		{
			...
			ICampaignMission current = CampaignMission.Current;
			Location val = ((current != null) ? current.Location : null);
			if (val == null || request.TargetSourceLocation == null)
			{
				return false;
			}
			if (request.TargetSourceLocation == val)
			{
				Agent val2 = ResolveAgentForLocationCharacter(request.TargetLocationCharacter);
				if (!CanAgentParticipateInSceneSpeech(val2))
				{
					return false;
				}
				EscortAgentBehavior.AddEscortAgentBehavior(guideAgent, val2, (OnTargetReachedDelegate)null);
				return true;
			}
			List<Location> list = FindSceneLocationPath(val, request.TargetSourceLocation);
			if (list == null || list.Count < 2)
			{
				return false;
			}
			Passage val3 = request.TargetDoorPassage ?? FindCurrentScenePassageToLocation(list[1]);
			if (val3 == null)
			{
				return false;
			}
			request.TargetDoorPassage = val3;
			return TryStartEscortBehavior(guideAgent, (UsableMachine)(object)val3);
		}
		catch
		{
			return false;
	...
	private bool TickSceneGuideRequest(ActiveSceneGuideRequest request, float currentTime)
	{
		...
		EscortAgentBehavior sceneGuideEscortBehavior = GetSceneGuideEscortBehavior(val);
		if (!request.EscortStarted || sceneGuideEscortBehavior == null)
		{
			if (!TryStartSceneGuideEscort(request, val))
			{
				return true;
			}
			request.EscortStarted = true;
			sceneGuideEscortBehavior = GetSceneGuideEscortBehavior(val);
		}
		ICampaignMission current2 = CampaignMission.Current;
		...
		bool reachedDoor = request.TargetSourceLocation != val2;
	...
	private void TriggerSceneGuideArrivalAndReturn(ActiveSceneGuideRequest request, bool reachedDoor)
	{
		...
		{
			Mission current = Mission.Current;
			Agent agent = ((current == null) ? null : ((IEnumerable<Agent>)current.Agents)?.FirstOrDefault((Agent a) => a != null && a.Index == request.GuideAgentIndex));
		...
		for (int num = _activeSceneGuideRequests.Count - 1; num >= 0; num--)
		{
			...
			if (activeSceneGuideRequest != null)
			{
				...
				if (((current != null) ? current.Location : null) == activeSceneGuideRequest.CurrentLocation)
				{
					...
					{
						Mission current2 = Mission.Current;
						StopSceneGuideEscort((current2 == null) ? null : ((IEnumerable<Agent>)current2.Agents)?.FirstOrDefault((Agent a) => a != null && a.Index == activeSceneGuideRequest?.GuideAgentIndex));
			...
			Mission current3 = Mission.Current;
			StopSceneGuideEscort((current3 == null) ? null : ((IEnumerable<Agent>)current3.Agents)?.FirstOrDefault((Agent a) => a != null && a.Index == activeSceneGuideRequest?.GuideAgentIndex));
			_activeSceneGuideRequests.RemoveAt(num);
		}
	}
	...
	private void PrepareAutoGroupParticipantsForIdleTimeout(List<NpcDataPacket> participants, string trailingSpeechText = null)
	{
		...
		List<NpcDataPacket> list = (from npc in participants
			where npc != null && npc.AgentIndex >= 0
			group npc by npc.AgentIndex into @group
		...
		foreach (NpcDataPacket item in list)
		{
			TrackPlayerInteraction(item, participantCount, 15f);
	...
	private void RefreshSceneConversationParticipantInteractions(List<NpcDataPacket> participants, float timeoutSeconds = -1f)
	{
		...
		foreach (NpcDataPacket item in list)
		{
			TrackPlayerInteraction(item, participantCount, timeoutSeconds);
		}
	}
	...
	private void FlushSceneAutonomyRestoreAfterSpeech(int agentIndex)
	{
		...
		Mission current = Mission.Current;
		Agent agent = ((current == null) ? null : ((IEnumerable<Agent>)current.Agents)?.FirstOrDefault((Agent a) => a != null && a.Index == agentIndex && a.IsActive()));
		if (CanAgentParticipateInSceneSpeech(agent))
		{
	...
	private void UpdatePendingSceneAutonomyRestoresAfterSpeech()
	{
		...
		float currentTime = Mission.Current.CurrentTime;
		List<int> list = null;
		foreach (KeyValuePair<int, PendingSceneAutonomyRestoreAfterSpeech> item in _pendingSceneAutonomyRestoresAfterSpeech)
		{
			PendingSceneAutonomyRestoreAfterSpeech value = item.Value;
			if (value != null && !value.WaitForPlaybackFinished && value.ExecuteAtMissionTime >= 0f && currentTime >= value.ExecuteAtMissionTime)
			{
				if (list == null)
				{
		...
		foreach (int item2 in list)
		{
			FlushSceneAutonomyRestoreAfterSpeech(item2);
		}
	}
	...
	private void UpdateActiveInteractionTimeouts()
	{
		...
		foreach (int item in list)
		{
			...
			{
				List<SceneInteractionSession> list2 = BuildGroupedExpiredInteractionSessions(value2, list);
				if (list2 != null && list2.Count > 1)
				{
					foreach (SceneInteractionSession item2 in list2)
					{
						if (item2 != null && item2.TargetAgentIndex >= 0)
						{
							hashSet.Add(item2.TargetAgentIndex);
						}
					}
					ExpireGroupedActiveInteractions(list2);
				}
				else
				{
	...
	private List<SceneInteractionSession> BuildGroupedExpiredInteractionSessions(SceneInteractionSession seedSession, List<int> expiredAgentIndices)
	{
		...
		Agent val = ((IEnumerable<Agent>)Mission.Current.Agents)?.FirstOrDefault((Agent a) => a != null && a.Index == seedSession.TargetAgentIndex && a.IsActive());
		if (val == null)
		{
			return null;
		}
		List<SceneInteractionSession> list = new List<SceneInteractionSession> { seedSession };
		for (int num = 0; num < expiredAgentIndices.Count; num++)
		{
			int num2 = expiredAgentIndices[num];
			if (num2 < 0 || num2 == seedSession.TargetAgentIndex || !_activeInteractionSessions.TryGetValue(num2, out var value) || value == null || value.TimeoutSeconds <= 3.5f || Math.Abs(value.TimeoutSeconds - seedSession.TimeoutSeconds) > 0.5f || Math.Abs(value.LastActivityTime - seedSession.LastActivityTime) > 1.5f)
			{
				continue;
			}
			Agent val2 = ((IEnumerable<Agent>)Mission.Current.Agents)?.FirstOrDefault((Agent a) => a != null && a.Index == num2 && a.IsActive());
			if (val2 != null)
			{
				Vec3 position = val.Position;
				if (!(((Vec3)(ref position)).DistanceSquared(val2.Position) > 36f))
				{
					list.Add(value);
				}
			}
	...
	private void ExpireGroupedActiveInteractions(List<SceneInteractionSession> sessions)
	{
		...
		List<SceneInteractionSession> list = (from s in sessions
			where s != null && s.TargetAgentIndex >= 0
			group s by s.TargetAgentIndex into @group
			select @group.First()).ToList();
		if (list.Count == 0)
		{
			return;
		}
		SceneInteractionSession sceneInteractionSession = ChooseGroupedTimeoutRepresentative(list);
		if (sceneInteractionSession == null)
		{
		...
		SceneSummonConversationSession sceneSummonConversationSession = TryGetSceneSummonConversationSessionForAgentIndex(sceneInteractionSession.TargetAgentIndex);
		foreach (SceneInteractionSession item2 in list)
		{
			if (item2 != null && item2.TargetAgentIndex >= 0 && item2.TargetAgentIndex != sceneInteractionSession.TargetAgentIndex)
			{
				bool deferSceneSummonReturn = sceneSummonConversationSession != null && TryGetSceneSummonConversationSessionForAgentIndex(item2.TargetAgentIndex) == sceneSummonConversationSession;
		...
		if (!TriggerGroupedTimeoutDisperseSpeech(sceneInteractionSession, list, sceneSummonConversationSession))
		{
			ExpireActiveInteractionSilently(sceneInteractionSession, deferSceneSummonReturn: false);
		}
		_activeInteractionSessions.Remove(sceneInteractionSession.TargetAgentIndex);
		_pendingInteractionTimeoutArms.Remove(sceneInteractionSession.TargetAgentIndex);
	}
	...
	private SceneInteractionSession ChooseGroupedTimeoutRepresentative(List<SceneInteractionSession> sessions)
	{
		...
		for (int i = 0; i < sessions.Count; i++)
		{
			...
			Agent val = ((IEnumerable<Agent>)Mission.Current.Agents)?.FirstOrDefault((Agent a) => a != null && a.Index == sceneInteractionSession2.TargetAgentIndex && a.IsActive());
			if (CanAgentParticipateInSceneSpeech(val) && !IsAgentHostileToMainAgent(val))
			{
				float num2;
				...
				{
					Vec3 position = main.Position;
					num2 = ((Vec3)(ref position)).DistanceSquared(val.Position);
				}
				float num3 = num2;
				if (sceneInteractionSession == null || num3 < num)
				{
	...
	private bool TriggerGroupedTimeoutDisperseSpeech(SceneInteractionSession representativeSession, List<SceneInteractionSession> sessions, SceneSummonConversationSession representativeSummonSession)
	{
		...
		Agent agent = ((IEnumerable<Agent>)Mission.Current.Agents)?.FirstOrDefault((Agent a) => a != null && a.Index == representativeSession.TargetAgentIndex && a.IsActive());
		...
		if (sessions != null)
		{
			foreach (SceneInteractionSession session in sessions)
			{
				...
				{
					Agent agent2 = ((IEnumerable<Agent>)Mission.Current.Agents)?.FirstOrDefault((Agent a) => a != null && a.Index == session.TargetAgentIndex && a.IsActive());
					NpcDataPacket npcDataPacket2 = ShoutUtils.ExtractNpcData(agent2);
					if (npcDataPacket2 != null && !list.Any((NpcDataPacket x) => x != null && x.AgentIndex == npcDataPacket2.AgentIndex))
					{
		...
		if (!list.Any((NpcDataPacket x) => x != null && x.AgentIndex == npcDataPacket.AgentIndex))
		{
			list.Insert(0, npcDataPacket);
		}
		string text = "如果没事，大家就散了。";
		string text2 = SanitizeSceneSpeechText(text);
		if (!string.IsNullOrWhiteSpace(text2))
		{
			RecordResponseForAllNearbySafe(list, npcDataPacket.AgentIndex, npcDataPacket.Name, text2);
			PersistNpcSpeechToNamedHeroes(npcDataPacket.AgentIndex, npcDataPacket.Name, text2, list);
		}
		...
		EnqueueSpeechLineWithOptions(npcDataPacket, text, list, commitHistory: false, suppressStare: false, allowPlayerDirectedActions: false, 0);
		return true;
	}
	...
	private void ExpireActiveInteractionSilently(SceneInteractionSession session, bool deferSceneSummonReturn)
	{
		...
		Mission current = Mission.Current;
		Agent agent = ((current == null) ? null : ((IEnumerable<Agent>)current.Agents)?.FirstOrDefault((Agent a) => a != null && a.Index == session.TargetAgentIndex && a.IsActive()));
