...
public class DuelSettings : AttributeGlobalSettings<DuelSettings>
{
	...
	public DuelSettings()
	{
		TestTtsVolcDedicatedVoice = delegate
		{
			...
			try
			{
				DuelSettings duelSettings = GetSettings() ?? this;
				if (duelSettings != this && (!string.Equals((TtsVolcDedicatedApiUrl ?? "").Trim(), (duelSettings.TtsVolcDedicatedApiUrl ?? "").Trim(), StringComparison.Ordinal) || !string.Equals((TtsVolcDedicatedApiKey ?? "").Trim(), (duelSettings.TtsVolcDedicatedApiKey ?? "").Trim(), StringComparison.Ordinal) || !string.Equals((TtsVolcDedicatedAppKey ?? "").Trim(), (duelSettings.TtsVolcDedicatedAppKey ?? "").Trim(), StringComparison.Ordinal) || !string.Equals((TtsVolcDedicatedResourceId ?? "").Trim(), (duelSettings.TtsVolcDedicatedResourceId ?? "").Trim(), StringComparison.Ordinal) || !string.Equals((TtsVolcDedicatedSpeaker ?? "").Trim(), (duelSettings.TtsVolcDedicatedSpeaker ?? "").Trim(), StringComparison.Ordinal)))
				{
			...
			Task.Run(async delegate
			{
				try
				{
					...
					else
					{
						...
						string jsonBody = JsonConvert.SerializeObject((object)requestPayload);
						...
						HttpResponseMessage response = await GlobalClient.PostAsync(effectiveApiUrl, (HttpContent)content);
						...
						if (response.IsSuccessStatusCode)
						{
							try
							{
								...
								JToken obj = jsonResponse["choices"];
								object obj2;
								if (obj == null)
								{
									obj2 = null;
				...
				catch (Exception ex)
				{
					Exception ex3 = ex;
					Exception ex4 = ex3;
					InformationManager.DisplayMessage(new InformationMessage("[系统] 异常: " + ex4.Message, Color.FromUint(4294901760u)));
					Logger.Log("DuelSettings", "测试崩溃: " + ex4.Message);
				}
			});
		};
	}
