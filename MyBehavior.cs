...
public class MyBehavior : CampaignBehaviorBase
{
	...
	private bool _pendingMissingOnnxGateCheck;

	private long _pendingMissingOnnxGateCheckAfterUtcTicks;
	...
	private int GetCurrentWeeklyIndexSafe()
	{
		int currentGameDayIndexSafe = GetCurrentGameDayIndexSafe();
	...
	private string BuildNpcIdentityInfoForPrompt(Hero npcHero, bool includeTradePricing, bool includeMarriageCandidates = false)
	{
		...
		if (hero != null)
		{
			...
			{
				stringBuilder.AppendLine("该家族族长：" + (hero.Name?.ToString() ?? "未知"));
			}
		}
		...
		if (includeTradePricing && RewardSystemBehavior.Instance != null)
		{
			try
			{
				string text7 = RewardSystemBehavior.Instance.BuildInventorySummaryForAI(npcHero, includePrivateBattleEquipment: includeTradePricing);
				if (!string.IsNullOrWhiteSpace(text7))
				{
					stringBuilder.AppendLine(BuildNpcInventorySummaryHeader(npcHero));
	...
	{
		string text = (npcHero?.Name?.ToString() ?? "").Replace("\r", "").Replace("\n", "").Trim();
		if (string.IsNullOrWhiteSpace(text))
		{
			text = "NPC";
		}
	...
	private string BuildNpcSystemTopPromptIntro(Hero npcHero, bool includeTradePricing)
	{
		...
		if (includeTradePricing && RewardSystemBehavior.Instance != null)
		{
			try
			{
				inventorySummary = RewardSystemBehavior.Instance.BuildInventorySummaryForAI(npcHero, includePrivateBattleEquipment: includeTradePricing);
			}
			catch
			{
				inventorySummary = "";
	...
	private static bool IsPartyTransferLordEligible(Hero targetHero, CharacterObject targetCharacter = null)
	{
	...
	private string TryRunDuelActionPostprocess(Hero targetHero, string extraFact, string replyText, List<RewardSystemBehavior.DuelStakeOption> duelStakeOptions)
	{
		...
		try
		{
			...
			{
				text7 = RewardSystemBehavior.Instance.BuildVisibleEquipmentPostprocessListForAI(Hero.MainHero);
			}
		}
		catch
		{
	...
	{
		string text = BuildWeeklyEpicWeekLabel(weekIndex);
		if (string.Equals((group?.GroupKind ?? "").Trim(), "world", StringComparison.OrdinalIgnoreCase))
		{
			return text + "世界周报";
		}
		string text2 = ResolveKingdomDisplay(group?.KingdomId);
		if (string.IsNullOrWhiteSpace(text2))
		{
			text2 = "某王国";
		}
		return text2 + text + "周报";
	}

	private static string BuildWeeklyEpicWeekLabel(int weekIndex)
	{
		if (weekIndex <= 0)
		{
			return "第0周";
		...
		const int num2 = 21;
		const int num3 = 84;
		int num4 = Math.Max(0, (weekIndex - 1) * num);
		int num5 = num4 / num3;
		int num6 = num4 % num3;
		int seasonIndexZeroBased = num6 / num2;
		int num7 = Math.Min(3, num6 % num2 / num + 1);
		string seasonTextZhForPrompt = GetSeasonTextZhForPrompt(seasonIndexZeroBased);
		return num5 + "年" + seasonTextZhForPrompt + "季" + ConvertWeekNumberToChineseOrdinal(num7) + "周";
	}

	private static string ConvertWeekNumberToChineseOrdinal(int weekNumber)
	{
		switch (weekNumber)
		{
		case 1:
	...
	private void TryNotifyNearestKingdomWeeklyReportGenerated(WeeklyEventMaterialPreviewGroup group, int weekIndex, IEnumerable<string> candidateKingdomIds, string title, string report, ref bool notified)
	{
		...
		notified = true;
		string text3 = ResolveKingdomDisplay(text);
		if (string.IsNullOrWhiteSpace(text3))
		{
			text3 = "最近王国";
		}
		...
		if (string.IsNullOrWhiteSpace(text4))
		{
			text4 = text3 + BuildWeeklyEpicWeekLabel(weekIndex) + "周报";
		}
