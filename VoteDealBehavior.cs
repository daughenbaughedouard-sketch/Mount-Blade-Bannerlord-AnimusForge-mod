...
namespace AnimusForge
{
	public partial class VoteDealBehavior : CampaignBehaviorBase
	{
		...
		private string ProcessTargetedVoteDealTag(Hero npc, string agendaCode, string optionCode, string weightStr, string notes)
		{
			try
			{
				...
				if (agenda.Decision.ProposerClan != null && clan.StringId == agenda.Decision.ProposerClan.StringId)
				{ Logger.Log("VoteDeal","Proposer"); return ""; }
	...
	public class AgendaOptionVM : ViewModel
	{
			...
			try
			{
				...
				switch (type)
				{
					case "WAR":
					{
						...
						if (tg == null)
						{ Logger.Log("ProposeAgenda", "WAR: faction not found: " + tgt); return ""; }
						if (!tg.IsKingdomFaction)
						{ Logger.Log("ProposeAgenda", "WAR: not a kingdom: " + tgt); return ""; }
						if (FactionManager.IsAtWarAgainstFaction(kd, tg))
						{ Logger.Log("ProposeAgenda", "WAR: already at war with " + tgt); return ""; }
						int cst = dm.GetInfluenceCostOfProposingWar(cl);
						if (cl.Influence < cst)
						{ Logger.Log("ProposeAgenda", "WAR: need " + cst + " influence, have " + (int)cl.Influence); return ""; }
						Logger.Log("ProposeAgenda", "WAR: creating DeclareWarDecision...");
				...
				AnimusForgeQuickInfo.ShowForDuration(
					cn + "家族 向王国提出了新议程", 6000, npc.CharacterObject);
		...
		private static IFaction FindFactionByName(string name)
		{
			...
			name = name.Trim();
			var allKingdoms = new List<string>();
			// Exact match
			foreach (Kingdom k in Kingdom.All)
			{
				allKingdoms.Add(k.StringId + "=" + (k.Name?.ToString() ?? "?"));
				if (string.Equals(k.Name?.ToString() ?? "", name, StringComparison.OrdinalIgnoreCase))
					return k;
				if (string.Equals(k.StringId, name, StringComparison.OrdinalIgnoreCase))
					return k;
			}
			// Contains match
			foreach (Kingdom k in Kingdom.All)
			{
				...
				if (!string.IsNullOrWhiteSpace(kn) && kn.Contains(name))
					return k;
			}
			// Also check FactionManager for non-kingdom factions
			foreach (Kingdom k in Kingdom.All)
			{
				if (string.Equals(k.StringId, name, StringComparison.OrdinalIgnoreCase))
					return k;
			}
			Logger.Log("ProposeAgenda", "FindFactionByName failed for '" + name + "'. Available kingdoms: " + string.Join(", ", allKingdoms));
			return null;
		}
		...
		private static PolicyObject FindPolicyByName(string name)
		{
			...
			foreach (PolicyObject p in PolicyObject.All)
			{
				if (string.Equals(p.Name?.ToString() ?? "", name, StringComparison.OrdinalIgnoreCase))
					return p;
				if (string.Equals(p.StringId, name, StringComparison.OrdinalIgnoreCase))
					return p;
			}
			foreach (PolicyObject p in PolicyObject.All)
			{
				string pn = p.Name?.ToString() ?? "";
				if (!string.IsNullOrWhiteSpace(pn) && pn.Contains(name))
					return p;
			}
			return null;
		}
		...
		private static Clan FindClanByName(string name, Kingdom kingdom)
		{
			...
			{
				if (string.Equals(cl.Name?.ToString() ?? "", name, StringComparison.OrdinalIgnoreCase))
					return cl;
				if (string.Equals(cl.StringId, name, StringComparison.OrdinalIgnoreCase))
					return cl;
			}
			foreach (Clan cl in kingdom.Clans)
			{
				string cn = cl.Name?.ToString() ?? "";
				if (!string.IsNullOrWhiteSpace(cn) && cn.Contains(name))
					return cl;
			}
			return null;
		}
		...
		private static Town FindTownByName(string name, Kingdom kingdom)
		{
			...
			{
				if (string.Equals(t.Name?.ToString() ?? "", name, StringComparison.OrdinalIgnoreCase))
					return t;
				if (string.Equals(t.StringId, name, StringComparison.OrdinalIgnoreCase))
					return t;
			}
			foreach (Town t in kingdom.Fiefs)
			{
				string tn = t.Name?.ToString() ?? "";
				if (!string.IsNullOrWhiteSpace(tn) && tn.Contains(name))
					return t;
			}
			return null;
		}
		...
		public static bool CanPropose(Hero npc)
		{
			...
			{
				var cl = npc?.Clan;
				return cl != null && cl.Kingdom != null
					&& !cl.IsUnderMercenaryService && npc == cl.Leader;
			}
			catch { return false; }
		}
		...
		public static string BuildProposeRuntimeInstruction(Hero npc)
		{
			try
			{
				...
				var tk = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
				{ ["playerName"] = pn };
				...
				if (kd != null && !cl.IsUnderMercenaryService && npc == cl.Leader)
				{
					...
					try
					{
						ti = RewardSystemBehavior.GetTrustLevelIndex(
							RewardSystemBehavior.Instance?.GetEffectiveTrust(npc) ?? 0);
					}
					catch { }
