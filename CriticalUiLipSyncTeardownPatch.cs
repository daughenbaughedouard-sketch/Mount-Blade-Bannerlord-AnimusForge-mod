...
public static class CriticalUiLipSyncTeardownPatch
{
	...
	public static void BeforePushState(GameState gameState)
	{
		try
		{
			ShoutBehavior.NotifyCriticalUiTransition("PUSHSTATE:" + (gameState?.GetType().Name ?? "null"));
		}
		catch
		{
		}
	...
	public static void BeforeCleanAndPushState(GameState gameState)
	{
		try
		{
			ShoutBehavior.NotifyCriticalUiTransition("CLEANPUSH:" + (gameState?.GetType().Name ?? "null"));
		}
		catch
		{
		}
