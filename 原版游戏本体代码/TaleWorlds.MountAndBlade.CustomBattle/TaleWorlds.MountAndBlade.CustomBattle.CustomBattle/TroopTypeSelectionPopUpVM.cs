...
public class TroopTypeSelectionPopUpVM : ViewModel
{
	...
	public MBBindingList<CustomBattleTroopTypeVM> Items
	{
		...
		set
		{
			...
			{
				_items = value;
				((ViewModel)this).OnPropertyChangedWithValue<MBBindingList<CustomBattleTroopTypeVM>>(value, "Items");
			}
		}
	}
	...
	public void OpenPopUp(string title, MBBindingList<CustomBattleTroopTypeVM> troops)
	{
		...
		foreach (CustomBattleTroopTypeVM item in (Collection<CustomBattleTroopTypeVM>)(object)troops)
		{
			_itemSelectionsBackUp.Add(item.IsSelected);
		}
		_selectedItemCount = ((IEnumerable<CustomBattleTroopTypeVM>)troops).Count((CustomBattleTroopTypeVM x) => x.IsSelected);
		Title = title;
		Items = troops;
		IsOpen = true;
	}
	...
	{
		Items.ApplyActionOnAllItems((Action<CustomBattleTroopTypeVM>)delegate(CustomBattleTroopTypeVM x)
		{
			x.IsSelected = true;
		});
		_selectedItemCount = ((Collection<CustomBattleTroopTypeVM>)(object)Items).Count;
	}
	...
	public void ExecuteBackToDefault()
	{
		...
		_selectedItemCount = ((IEnumerable<CustomBattleTroopTypeVM>)Items).Count((CustomBattleTroopTypeVM x) => x.IsSelected);
	}
	...
	public void ExecuteReset()
	{
		...
		if (count != ((Collection<CustomBattleTroopTypeVM>)(object)Items).Count)
		{
		...
		{
			((Collection<CustomBattleTroopTypeVM>)(object)Items)[i].IsSelected = _itemSelectionsBackUp[i];
		}
		_selectedItemCount = ((IEnumerable<CustomBattleTroopTypeVM>)Items).Count((CustomBattleTroopTypeVM x) => x.IsSelected);
	}
