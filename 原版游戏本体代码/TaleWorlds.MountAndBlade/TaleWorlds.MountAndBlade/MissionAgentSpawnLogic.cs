...
public class MissionAgentSpawnLogic : MissionLogic, IMissionAgentSpawnLogic, IMissionBehavior
{
	...
	private class MissionSide
	{
		...
		public void GetTeamFormationsSpawnData(out MBList<(Team team, FormationSpawnData[] formationSpawnData)> teamFormationsSpawnData)
		{
			teamFormationsSpawnData = new MBList<(Team, FormationSpawnData[])>();
	...
	private bool CheckDeployment()
	{
		...
		if (!isDeploymentOver)
		{
			...
			for (int i = 0; i < 2; i++)
			{
				...
				_missionSides[i].GetTeamFormationsSpawnData(out MBList<(Team, FormationSpawnData[])> teamFormationsSpawnData);
				if (base.Mission.HasSpawnPath)
				{
					MBReadOnlyList<(Team, int, FormationSpawnData[])> mBReadOnlyList = CollectSortedBattleSideTeamsData(teamFormationsSpawnData);
					...
					{
						array[j] = GetTeamSpawnPathOffsetRange(path, battleSideEnum, mBReadOnlyList[j].Item3);
					}
					Path path2 = initialSpawnPathData.Path;
					float baseDeploymentOffset = Mission.ComputeSpawnPathDeploymentOffset(troopCount, path2);
					...
					{
						(Team, int, FormationSpawnData[]) tuple = mBReadOnlyList[k];
			...
			for (int l = 0; l < 2; l++)
			{
				...
				if (!_missionSides[l].TroopSpawnActive)
				{
					continue;
				}
				int reservedTroopsCount = _missionSides[l].ReservedTroopsCount;
	...
	private float GetTeamSpawnPathOffsetRange(Path initialSpawnPath, BattleSideEnum battleSide, FormationSpawnData[] teamFormationsSpawnData)
	{
		float centerFlanksDepth = 0f;
		float leftFlanksDepth = 0f;
		float rightFlanksDepth = 0f;
		float num = 3f;
		bool teamPlanHasAnyFootTroops = teamFormationsSpawnData.Sum((FormationSpawnData data) => data.FootTroopCount) > 0;
		for (int num2 = 0; num2 < teamFormationsSpawnData.Count(); num2++)
		{
			FormationSpawnData formationSpawnData = teamFormationsSpawnData[num2];
			FormationClass formationClass = (FormationClass)num2;
			if (formationSpawnData.NumTroops > 0)
			{
				bool flag = DefaultMissionDeploymentPlan.HasSignificantMountedTroops(formationSpawnData.FootTroopCount, formationSpawnData.MountedTroopCount);
				float item = DefaultDeploymentPlan.GetFormationSpawnWidthAndDepth(formationClass, formationSpawnData.NumTroops, flag, !GetSpawnHorses(battleSide)).Item2;
				ref float flankDepthRef = ref GetFlankDepthRef(DefaultFormationDeploymentPlan.GetFormationDefaultFlankAux(formationClass, formationSpawnData.NumTroops, teamPlanHasAnyFootTroops, flag, GetSpawnHorses(battleSide)), ref centerFlanksDepth, ref leftFlanksDepth, ref rightFlanksDepth);
	...
	private MBReadOnlyList<(Team team, int deployingTroopCount, FormationSpawnData[] formationSpawnData)> CollectSortedBattleSideTeamsData(MBList<(Team team, FormationSpawnData[] formationSpawnData)> teamFormationsSpawnData)
	{
		MBList<(Team, int, FormationSpawnData[])> mBList = new MBList<(Team, int, FormationSpawnData[])>();
		foreach (var teamFormationsSpawnDatum in teamFormationsSpawnData)
		{
			int num = 0;
			FormationSpawnData[] item = teamFormationsSpawnDatum.formationSpawnData;
			foreach (FormationSpawnData formationSpawnData in item)
			{
				num += formationSpawnData.NumTroops;
			}
			if (num > 0)
			{
		...
		mBList.Sort(delegate((Team team, int deployingTroopCount, FormationSpawnData[] formationSpawnData) t1, (Team team, int deployingTroopCount, FormationSpawnData[] formationSpawnData) t2)
		{
			...
			if (flag && flag2)
			{
				Debug.FailedAssert("There can be only one main team in a battle side", "C:\\BuildAgent\\work\\mb3\\Source\\Bannerlord\\TaleWorlds.MountAndBlade\\Missions\\MissionLogics\\MissionAgentSpawnLogic.cs", "CollectSortedBattleSideTeamsData", 1725);
				return 0;
			}
			if (!flag && !flag2)
			{
