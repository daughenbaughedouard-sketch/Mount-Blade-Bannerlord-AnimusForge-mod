...
public class BasicCharacterObject : MBObjectBase
{
	...
	public Equipment GetFirstBaseBattleEquipmentForLoad()
	{
		return _equipmentRoster?.AllEquipments.FirstOrDefaultQ((Equipment t) => t.IsBattle);
	}

	public Equipment GetFirstBaseCivilEquipmentForLoad()
	{
		return _equipmentRoster?.AllEquipments.FirstOrDefaultQ((Equipment t) => t.IsCivilian);
	}
			...
			else if (childNode.Name == "face")
			{
				foreach (XmlNode childNode4 in childNode.ChildNodes)
				{
					...
					else if (childNode4.Name == "BodyProperties")
					{
						...
						{
							Debug.FailedAssert("cannot read body properties", "C:\\BuildAgent\\work\\mb3\\Source\\Bannerlord\\TaleWorlds.Core\\BasicCharacterObject.cs", "Deserialize", 519);
						}
					}
					else if (childNode4.Name == "BodyPropertiesMax")
					{
						if (!BodyProperties.FromXmlNode(childNode4, out bodyProperties2))
						{
							...
							Debug.FailedAssert("cannot read max body properties", "C:\\BuildAgent\\work\\mb3\\Source\\Bannerlord\\TaleWorlds.Core\\BasicCharacterObject.cs", "Deserialize", 528);
						}
					}
