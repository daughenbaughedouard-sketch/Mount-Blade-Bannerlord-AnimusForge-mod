...
public sealed class Hero : MBObjectBase, ITrackableCampaignObject, ITrackableBase, IRandomOwner
{
	...
	protected override void AfterLoad()
	{
		...
		if (!MBSaveLoad.IsUpdatingGameVersion || !MBSaveLoad.LastLoadedGameVersion.IsOlderThan(ApplicationVersion.FromString("v1.3.16")))
		{
			return;
		}
		bool flag = false;
		...
		for (int num3 = 5; num3 < 12; num3++)
		{
			...
			if (IsFemale)
			{
				if (equipmentFromSlot.Item != null && equipmentFromSlot.Item.ItemFlags.HasAnyFlag(ItemFlags.NotUsableByFemale))
				{
					flag = true;
				}
				if (equipmentFromSlot2.Item != null && equipmentFromSlot2.Item.ItemFlags.HasAnyFlag(ItemFlags.NotUsableByFemale))
				{
					flag2 = true;
				}
			}
			...
			{
				if (equipmentFromSlot.Item != null && equipmentFromSlot.Item.ItemFlags.HasAnyFlag(ItemFlags.NotUsableByMale))
				{
					flag = true;
				}
				if (equipmentFromSlot2.Item != null && equipmentFromSlot2.Item.ItemFlags.HasAnyFlag(ItemFlags.NotUsableByMale))
				{
					flag2 = true;
				}
			}
		...
		if (flag)
		{
			Equipment equipment = CharacterObject.GetFirstBaseBattleEquipmentForLoad();
			if (equipment == null || equipment.IsEmpty())
			{
				equipment = Culture.DefaultBattleEquipmentRoster.DefaultEquipment;
			}
			_battleEquipment = equipment;
		}
		if (flag2)
		{
			Equipment equipment2 = CharacterObject.GetFirstBaseCivilEquipmentForLoad();
			if (equipment2 == null || equipment2.IsEmpty())
			{
				equipment2 = Culture.DefaultCivilianEquipmentRoster.DefaultEquipment;
			}
			_civilianEquipment = equipment2;
		}
	}
	...
	public bool CanBecomePrisoner()
	{
		...
		{
			Debug.FailedAssert("Only lords, companions and special quest heroes can become prisoners! Check CanBecomePrisoner usage.", "C:\\BuildAgent\\work\\mb3\\Source\\Bannerlord\\TaleWorlds.CampaignSystem\\Hero.cs", "CanBecomePrisoner", 1957);
			return false;
		}
		if (this != MainHero)
		{
