...
function Resolve-Bannerlord14ReferenceDir {
    param(
        [Parameter(Mandatory = $true)][string]$Root,
        [Parameter(Mandatory = $true)][string]$InstalledGameReferenceDir,
        [string]$RequestedDir
    )
    ...
    if (-not [string]::IsNullOrWhiteSpace($RequestedDir)) {
        ...
        $candidates.Add((Get-FullPathSafe -Path $InstalledGameReferenceDir))
        $candidates.Add((Get-FullPathSafe -Path (Join-Path $Root "_deps_1_4")))
        $candidates.Add((Get-FullPathSafe -Path (Join-Path $Root ".tmp\build_check\1.4")))
        $candidates.Add((Get-FullPathSafe -Path (Join-Path $Root ".tmp\build_new\1.4")))
    }

    $errors = New-Object System.Collections.Generic.List[string]
    foreach ($candidate in @($candidates | Select-Object -Unique)) {
        try {
            $version = Assert-ReferenceApiLine -ReferenceDir $candidate -ExpectedMinor 4 -Label "Bannerlord 1.4 implementation" -RequiredFiles @(
                "Newtonsoft.Json.dll",
                "SandBox.View.dll",
                "SandBox.ViewModelCollection.dll",
                "TaleWorlds.Library.dll",
            ...
            Write-Host "1.4 References: $($version.Directory) [$($version.Text)]"
            return $version.Directory
        }
        catch {
            $errors.Add($_.Exception.Message)
    ...
    throw "A verified Bannerlord 1.4.x reference directory is required. Pass -Bannerlord14ReferenceDir when the installed game is on another API line.`n$($errors -join "`n")"
}
...
function Invoke-DotNetBuild {
    ...
    if (-not [string]::IsNullOrWhiteSpace($VersionedReferenceDir)) {
        ...
        elseif (-not [string]::IsNullOrWhiteSpace($BannerlordApi)) {
            # A saved 1.4 overlay must replace every game/module reference directory;
            # otherwise a 1.3 installation can silently leak assemblies into the 1.4 build.
...
$installedGameReferenceDir = Join-Path $bannerlordRootFull "bin\Win64_Shipping_Client"
$bannerlord14ReferenceDirFull = Resolve-Bannerlord14ReferenceDir -Root $projectRootFull -InstalledGameReferenceDir $installedGameReferenceDir -RequestedDir $Bannerlord14ReferenceDir
