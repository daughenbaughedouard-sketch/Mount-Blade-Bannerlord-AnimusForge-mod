...
public static class Patch_GameMenu_SwitchToMenu_AFResolvedSiegeAftermath
{
	public static bool Prefix(ref string menuId)
	{
		try
		{
			if (SiegeAiInterventionBehavior.TryHandleDirectMassacreAftermathMenuForExternal(menuId, "GameMenu.SwitchToMenu:" + (menuId ?? "N/A")))
			{
				return false;
			}
			if (SiegeAiInterventionBehavior.TryHandleDirectPlunderAftermathMenuForExternal(menuId, "GameMenu.SwitchToMenu:" + (menuId ?? "N/A")))
			{
				return false;
			}
			...
			{
				Logger.LogTrace("UI_Intercept", "Suppressed native siege aftermath SwitchToMenu activation after GCCZ intervention. Menu=" + (menuId ?? "N/A"));
				return false;
			}
		}
		catch (Exception ex)
		{
			Logger.LogTrace("UI_Intercept", "[ERROR] SwitchToMenu GCCZ siege aftermath guard: " + ex);
		}
		return true;
	}
}
...
public static class Patch_GameStateManager_OnTick_AFMassacreLoot
{
	public static void Postfix(float dt)
	{
		...
		{
			Logger.LogTrace("UI_Intercept", "[ERROR] GameStateManager GCCZ aftermath loot pump: " + ex.Message);
		}
	}
}
