...
`SiegeCivilianGatherInteractionProfile` owns dependency-free runtime parameters and source codes for GCCZ 民众召集 messenger speech, follow refresh, fallback timing, approach distance, soldier messenger ratio, messenger speed, formation-control batching, target waiting, messenger movement, and follower preparation. AF adapters still own live mission-agent selection, `ShoutBehavior` triggering, movement, formation control, and side effects.
