...
public static class SiegePostprocessContextBuilder
{
    ...
    public static string Build(SiegePostprocessContextFacts facts)
    {
        ...
        sb.AppendLine("- 标签编号：1宽恕，2救济，3宣抚，4盟誓，5安兵，6召集，7抢钱，8搜掠，9血洗，10殖民；输出格式只用 [ACTION:数字]。");
        sb.AppendLine("- 若AF给予共享物资已经存在，且平民/商人/工匠/要人的回复谈到粮食、原料、钱货、供应、商路、工坊、修缮、安置、发放或救济，应优先判为[ACTION:2]；不要只判为[ACTION:1]。");
        sb.AppendLine("- [ACTION:1] 是单方处置：玩家明确宣布不杀不抢/不追究/放过民众即可，不需要平民同意；区域冲突造成的不信任不阻止宽恕。");
        sb.AppendLine("- 区域冲突后的不信任会让 [ACTION:2]/[ACTION:3]/[ACTION:4] 更难成立：只有 NPC 回复明确接受、传达或配合更高安抚安排时才输出这些标签；若只是玩家单方说不杀不抢，输出宽恕。");
        ...
        sb.AppendLine("- [ACTION:8]/[ACTION:9]/[ACTION:10] 是士兵执行型破坏性处置，只能由“玩家己方入城士兵”在“直接回应玩家本轮明确命令”时输出；士兵与士兵、士兵与平民、平民与平民的自然聊天，即使提到这些词，也不得输出这三个标签，只能自然请示、质疑、劝告或转述。");
        ...
        sb.AppendLine("- [ACTION:2] 分流：当前说话者是己方士兵时，必须已有AF共享物资且本轮明确分发给民众；当前说话者是战败平民/商人/工匠/镇民时，可以由玩家直接言语安抚、保护承诺、军纪约束、安顿安排，或围绕共享物资解决粮食/原料/钱货/供应困境触发。");
        sb.AppendLine("- 如果只是让己方士兵分发已交付的共享粮食、物资或第纳尔，最高只输出救济，不输出宣抚或盟誓。");
        sb.AppendLine("- 抢钱/搜掠分流：[ACTION:7] 只用于当前战败平民、商人、工匠、头人或要人直接回应玩家本轮向其索取第纳尔、货物、粮食或物资；NPC闲聊、转述、即时请示或非直接回应玩家时不要输出抢钱标签。[ACTION:8] 只用于玩家直接命令己方士兵组织全城搜掠。平民/商人/头人/要人禁止输出搜掠，己方士兵禁止输出抢钱。");
