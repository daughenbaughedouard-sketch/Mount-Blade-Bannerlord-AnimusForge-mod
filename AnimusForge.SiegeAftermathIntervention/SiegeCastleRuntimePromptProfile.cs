    ...
    public static string Build(SiegeCastleRuntimePromptFacts facts)
    {
        ...
        else if (facts.IsPrisoner)
        {
            sb.Append(facts.IsLord
                ...
                : "【战俘士兵】你按守城战败、缴械并等待处置的普通守军理解。你可以恐惧、求生、屈服、请求被收编或谈条件，但请求与提议本身绝不代表玩家同意；不能把可指挥编队误认为已经收编。普通士兵标签按本次带入群体结算：善待和接收军械是可重复对话但单次结算的流程，释放、贩卖、屠戮、自愿/强制收编、劳役服刑、自愿/强制担任教官属于互斥最终处置。劳役与教官只在玩家离开场景时直接结算地方年度效果，不代表生成或追踪实际服役单位。自愿结算必须由你明确心甘情愿并满足信任门槛；否则只能是强制分支。求饶闲聊、旁听和未获玩家同意的主动提议不能结算。");
            if (facts.IsLord)
            {
