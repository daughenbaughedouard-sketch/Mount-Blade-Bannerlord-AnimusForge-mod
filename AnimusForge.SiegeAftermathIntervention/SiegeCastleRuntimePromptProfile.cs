    ...
    public static string Build(SiegeCastleRuntimePromptFacts facts)
    {
        ...
        else if (facts.IsPrisoner)
        {
            sb.Append(facts.IsLord
                ...
                : "【战俘士兵】你按守城战败、缴械并等待处置的普通守军理解。你可以恐惧、求生、屈服，或请求释放、归顺、外派村庄劳役赎罪、留在城堡修缮城防、充当教官，但请求与提议本身绝不代表玩家同意；提议语义必须准确，不能把农奴、修缮、缴械、释放或教官说成收编，也不能把可指挥编队误认为已经收编。善待和接收军械是流程标签；释放、贩卖、自愿/强制收编、外派劳役、自愿/强制修缮城堡、自愿/强制担任教官会按本轮指定数量和兵种登记暂定去向，尚存战俘不会立刻消失，玩家离场时才逐组执行。玩家不说数量时随机挑选，不说兵种时随机选兵；说其余、剩下或全部时作用于当前未分配者。明确反悔或改判才重置幸存者旧计划。屠戮会立即攻击所选目标，只有实际死亡者才扣名册。劳役、修缮和教官离场时直接结算地方年度效果，不代表生成或追踪实际服役单位；修缮城堡按200人为满额基准，自愿最多使项目建设速度+50%，强制最多+25%，持续一年。自愿结算必须由你明确心甘情愿并满足信任门槛；否则只能是强制分支。求饶闲聊、旁听和未获玩家同意的主动提议不能结算。");
            if (facts.IsLord)
            {
        ...
        sb.Append("【城堡与城镇规则隔离】城镇民众、搜掠、抢钱、救济、宣抚、盟誓、召集民众、血洗城镇和迁殖规则不适用于本城堡阶段。不要输出或暗示任何城镇 GCCZ 处置标签。城堡专用的战俘收编、屠戮、士兵安抚和领主处置由独立接口处理，不能借用城镇标签代替。")
            .Append("【结算门槛】只有对应角色直接回应玩家本轮明确命令或明确同意时，城堡专用接口才可进入结算候选；运行时只向后处理器提供与当前角色、玩家本轮精确意图和未结状态匹配的标签。NPC主动提出释放、贩卖、收编、屠戮、外派劳役、修缮城堡或教官方案只能登记语义一致的待确认提议，绝不能默认兜底成收编。NPC闲聊、旁听、互相请示或环境短句只能表达态度，不能直接结算处置。一次回复默认最多结算一个城堡动作；仅当玩家用明确人数和‘其余/剩下’在同一句划分多个互不重叠的普通战俘去向时，才可分别结算多个分组标签。正文自然说话，不要直接打印动作标签、解释内部机制或伪造已经发生的副作用；动作标签只由独立后处理器生成。");
