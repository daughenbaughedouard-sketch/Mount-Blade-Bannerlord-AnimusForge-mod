...
Follow-up bridge fix: repeated AI-selected `[ACTION:召集]` from an allied soldier is now treated as a command-control cleanup signal when civilian gather is already pending or complete. The fused adapter delegates the accept/deny decision to `SiegeCivilianGatherInteractionProfile.ShouldReleaseSoldiersForCommandControlRepeat(...)`, then only returns/unlocks allied soldiers to the player infantry formation; it does not restart civilian gathering, spawn civilians, or add deterministic tags.
