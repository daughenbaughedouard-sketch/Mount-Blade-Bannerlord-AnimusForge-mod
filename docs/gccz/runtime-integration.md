...
Follow-up isolation: mercy profile selection now uses `SiegeMercyChoiceProfile`; the fused AF adapter delegates stop-plunder reason, soldier appeasement reason, shared-pool effect reason, message text, memory text, and loyalty bonus to the isolated GCCZ source area before applying Bannerlord side effects.
