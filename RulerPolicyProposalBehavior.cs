...
public static class RulerPolicyProposalBehavior
{
	public const string ActionTag = "[ACTION:RULER_POLICY_PROPOSAL]";

	private const int MaxHistoryChars = 4800;
	private const int ReservationLifetimeMinutes = 12;
	private static readonly Regex ActionTagRegex = new Regex("\\[ACTION:RULER_POLICY_PROPOSAL\\]", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant | RegexOptions.Compiled);
	...
	public static bool IsEligibleTargetForExternal(Hero ruler, out string failureReason)
	{
		...
		{
			failureReason = "统治者政策提议资格检查失败：" + ex.Message;
			return false;
		}
	}
	...
	public static List<PostprocessRuleEntry> BuildRuntimePostprocessRulesForExternal(Hero ruler)
	{
		...
		{
			Logger.Log("RulerPolicyProposal", "[PostprocessRules] eligible=false ruler=" + SafeHeroId(ruler) + " reason=" + (failureReason ?? ""));
			return result;
		}

		Kingdom kingdom = ruler.Clan?.Kingdom;
		string runtimeFact = "【运行时资格事实】代码已经确认：当前 NPC 英雄ID=" + SafeHeroId(ruler)
			+ "，王国ID=" + SafeKingdomId(kingdom)
			+ "；该 NPC 存活，所属王国未灭亡，并且就是该王国当前统治者。不得再次猜测或否定其统治者资格。";
		foreach (PostprocessRuleEntry rule in AIConfigHandler.GetGuardrailRulePostprocessRules("ruler_policy_proposal") ?? new List<PostprocessRuleEntry>())
		{
			...
			{
				Tag = ActionTag,
				Description = ((rule.Description ?? "").Trim() + " " + runtimeFact).Trim()
			});
		}
		Logger.Log("RulerPolicyProposal", "[PostprocessRules] eligible=true ruler=" + SafeHeroId(ruler)
			+ " kingdom=" + SafeKingdomId(kingdom) + " rules=" + result.Count);
		return result;
	}

	public static bool TryProcessAcceptedTag(Hero ruler, string chainName, string playerProposalText, string npcReplyText, ref string content, out string failureReason)
	{
		failureReason = "";
		...
		try
		{
			...
			try
			{
				InformationManager.DisplayMessage(new InformationMessage("统治者已接受建议，正在拟定政策并准备提交 AF 议程。", Colors.Green));
			}
			catch (Exception ex)
			{
		...
		{
			ReleaseReservation(scopeKey, requestKey);
			failureReason = "统治者政策建议入队失败：" + ex.Message;
